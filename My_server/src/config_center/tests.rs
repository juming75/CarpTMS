...
mod tests {
    ...
    #[test]
    fn test_file_config_storage() {
        // 创建临时目录
        let temp_dir = tempdir().unwrap();
        let temp_path = temp_dir.path().to_str().unwrap();

        // 创建文件存储
        let storage = FileConfigStorage::new(temp_path, None).unwrap();
        let storage = Arc::new(storage);
        ...
        // 验证文件已创建
        let safe_key = config_key.full_key().replace(['/', '\\', ':', '*', '?', '"', '<', '>', '|'], "_");
        let file_path = format!("{}/{}.json", temp_path, safe_key);
        assert!(std::path::Path::new(&file_path).exists());

        // 获取配置
        let retrieved_entry = storage.get(&config_key).unwrap();
        assert!(retrieved_entry.is_some());
        assert_eq!(retrieved_entry.unwrap().value.as_string().unwrap(), "test-value");
    ...
    fn test_storage_factory() {
        ...
        assert!(storage.store(&ConfigEntry {
            key: ConfigKey::new("test", "key"),
            value: ConfigValue::from_string("value"),
            created_at: Instant::now(),
            updated_at: Instant::now(),
            description: None,
            tags: vec![],
    ...
    fn test_storage_config() {
        ...
        assert_eq!(memory_config.storage_type, StorageType::Memory);

        let file_config = StorageConfig::file("/tmp/config", None);
        assert_eq!(file_config.storage_type, StorageType::File);

        let redis_config = StorageConfig::redis("redis://localhost:6379", "config");
        assert_eq!(redis_config.storage_type, StorageType::Redis);
    }

    #[tokio::test]
    async fn test_composite_storage() {
        // 创建本地和远程存储
        let local_storage = Arc::new(MemoryConfigStorage::new(None));
        let remote_storage = Arc::new(MemoryConfigStorage::new(None));

        // 创建复合存储
        let mut composite = CompositeConfigStorage::new(local_storage.clone(), Some(remote_storage.clone()));

        // 创建测试配置
        let config_key = ConfigKey::new("test", "composite-key");
        let config_entry = ConfigEntry {
            key: config_key.clone(),
            value: ConfigValue::from_string("composite-value"),
            created_at: Instant::now(),
            updated_at: Instant::now(),
            description: Some("Composite test".to_string()),
            tags: vec!["composite"],
            version: "1.0.0".to_string(),
            status: ConfigStatus::Active,
            config_type: ConfigType::Custom,
        ...
        // 存储配置 - 应该同时存储在本地和远程
        composite.store(&config_entry).unwrap();

        // 验证本地存储包含配置
        let local_result = local_storage.get(&config_key).unwrap();
        assert!(local_result.is_some());

        // 验证远程存储包含配置
        let remote_result = remote_storage.get(&config_key).unwrap();
        assert!(remote_result.is_some());

        // 禁用同步
        composite.set_sync_enabled(false);

        // 创建另一个配置 - 应该只存储在本地
        let config_key2 = ConfigKey::new("test", "another-key");
        let config_entry2 = ConfigEntry {
            key: config_key2.clone(),
            value: ConfigValue::from_string("another-value"),
            created_at: Instant::now(),
            updated_at: Instant::now(),
            description: None,
            tags: vec![],
        ...
        composite.store(&config_entry2).unwrap();

        // 验证本地存储有，远程存储没有
        assert!(local_storage.get(&config_key2).unwrap().is_some());
        assert!(remote_storage.get(&config_key2).unwrap().is_none());
    }
}
