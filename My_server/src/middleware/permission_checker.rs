...
use crate::utils::permissions::{has_permission, Role as PermRole, Resource, Action};
...
pub fn role_from_str(role_str: &str) -> Role {
    ...
    if lowercase_role == "admin" || lowercase_role.starts_with("admin_") || lowercase_role.contains("管理员") {
        info!("Role mapping result: Admin");
        Role::Admin
    } else if lowercase_role == "manager" || lowercase_role.starts_with("manager_") || lowercase_role.contains("经理") {
        info!("Role mapping result: Manager");
        Role::Manager
    } else if lowercase_role == "user" || lowercase_role.starts_with("user_") || lowercase_role.contains("普通用户") {
