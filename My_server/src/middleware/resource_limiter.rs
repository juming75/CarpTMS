            ...
            if let Ok(socket_addr) = addr.parse::<SocketAddr>() {
                if let Ok(mut connections) = self.concurrent_connections.lock() {
                    ...
                    if connections.len() >= self.config.max_concurrent_connections {
                        return Err(AppError::service_unavailable_error("Too many concurrent connections", None));
                    }
...
pub fn resource_limiter_middleware_with_config(config: ResourceLimiterConfig) -> ResourceLimiterMiddleware {
