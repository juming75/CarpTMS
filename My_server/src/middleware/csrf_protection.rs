...
impl CsrfToken {
    ...
    pub fn verify(&self, provided_token: &str) -> bool {
        !self.is_expired() && bool::from(self.token.as_bytes().ct_eq(provided_token.as_bytes()))
    }
}
    ...
    B: 'static,
{
    ...
    fn call(&self, req: ServiceRequest) -> Self::Future {
        ...
        Box::pin(async move {
            ...
            if !bool::from(provided_token.as_bytes().ct_eq(cookie_token.as_bytes())) {
...
fn extract_csrf_token(req: &ServiceRequest, config: &CsrfConfig) -> Result<String> {
    ...
    if let Some(query_string) = req.uri().query() {
        for (key, value) in form_urlencoded::parse(query_string.as_bytes()) {
            let key: std::borrow::Cow<str> = key;
            let value: std::borrow::Cow<str> = value;
            let key_str: &str = key.as_ref();
            if key_str == config.token_name.as_str() {
                let token_str: String = value.into_owned();
                return Ok(token_str);
            }
        }
    }
