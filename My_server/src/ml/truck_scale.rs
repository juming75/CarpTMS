...
impl MLModel for TruckScalePredictionModel {
    async fn train(
        ...
        // 更新模型权重和偏置
        let vehicle_types: Vec<String> = self.weights.keys().map(|k: &String| k.to_string()).collect();
        for vehicle_type in vehicle_types {
            self.weights.insert(vehicle_type.clone(), weights.clone());
            self.biases.insert(vehicle_type.clone(), bias);
        }
    ...
    async fn predict(&self, features: &Array2<f32>) -> Result<Array1<f32>, MLError> {
        ...
        for sample in features.rows() {
            // 简单的线性预测
            let normalized = self.normalize_features(&sample.to_owned().into_owned());
