...
pub struct VideoStreamManager {
    ...
    /// 帧率统计 (stream_id -> FrameRateStats)
    framerate_stats: Arc<RwLock<HashMap<String, FrameRateStats>>>,
    /// 总帧率（所有流的聚合帧率）
    total_framerate: Arc<RwLock<u64>>,
}
...
impl VideoStreamManager {
    ...
    pub fn new() -> Self {
        Self {
            ...
            framerate_stats: Arc::new(RwLock::new(HashMap::new())),
            total_framerate: Arc::new(RwLock::new(0)),
        }
    }
    ...
    pub async fn distribute_frame(
        ...
        // 更新帧率统计
        self.update_framerate(stream_id).await;
    ...
    /// 更新帧率统计
    async fn update_framerate(&self, stream_id: &str) {
        let now = std::time::Instant::now();
        let mut framerate_stats = self.framerate_stats.write().await;

        if let Some(stats) = framerate_stats.get_mut(stream_id) {
            stats.total_frames += 1;
            let elapsed = now.duration_since(stats.last_update).as_secs_f64();
            if elapsed >= 1.0 {
                stats.current_fps = (stats.total_frames as f64) / elapsed;
                stats.total_frames = 0;
                stats.last_update = now;
            }
        } else {
            framerate_stats.insert(
                stream_id.to_string(),
                FrameRateStats {
        ...
        // 更新总帧率
        let total_fps: f64 = framerate_stats.values().map(|s| s.current_fps).sum();
        let mut total = self.total_framerate.write().await;
        *total = total_fps as u64;
    }

    /// 获取总帧率 (fps)
    pub async fn get_total_framerate(&self) -> u64 {
        *self.total_framerate.read().await
    }

    /// 获取指定流的帧率
    pub async fn get_stream_framerate(&self, stream_id: &str) -> f64 {
        let framerate_stats = self.framerate_stats.read().await;
        framerate_stats
            .get(stream_id)
            .map(|s| s.current_fps)
            .unwrap_or(0.0)
    ...
    /// 获取所有流的帧率信息
    pub async fn get_all_framerates(&self) -> Vec<(String, f64)> {
        let framerate_stats = self.framerate_stats.read().await;
        framerate_stats
            .iter()
            .map(|(id, s)| (id.clone(), s.current_fps))
            .collect()
    }
}
