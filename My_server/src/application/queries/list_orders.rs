...
impl QueryHandler<ListOrdersQuery, PagedResult<OrderDto>> for ListOrdersQueryHandler {
    async fn handle(&self, query: ListOrdersQuery) -> AppResult<PagedResult<OrderDto>> {
        ...
        let total: i64 = sqlx::query_scalar(&count_sql)
            .fetch_one(&self.pool)
            .await
            .map_err(|e| crate::errors::AppError::db_error(&format!("Failed to count orders: {}", e), None))?;
        ...
        let rows = sqlx::query(&data_sql)
            ...
            .fetch_all(&self.pool)
            .await
            .map_err(|e| crate::errors::AppError::db_error(&format!("Failed to list orders: {}", e), None))?;
