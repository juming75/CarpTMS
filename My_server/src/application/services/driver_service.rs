...
impl DriverService {
    ...
    pub async fn get_drivers(
        ...
        self.repository
            .find_all(page, page_size, query)
            .await
            .map_err(|e| AppError::internal_error(e.to_string().as_str(), None))
    }
    ...
    pub async fn create_driver(&self, request: DriverCreateRequest) -> Result<Driver, AppError> {
        ...
        let existing_count = self
            .repository
            .count_by_name(&request.driver_name, None)
            .await
            .map_err(|e| AppError::internal_error(e.to_string().as_str(), None))?;
        if existing_count > 0 {
