...
impl OrderApplicationService {
    ...
    pub async fn delete_order(&self, order_id: i32) -> AppResult<bool> {
        ...
        sqlx::query("DELETE FROM order_items WHERE order_id = $1")
            ...
            .await
            .map_err(|e| {
                AppError::db_error(&format!("Failed to delete order items: {}", e), None)
            })?;
    ...
    pub async fn get_order(&self, order_id: i32) -> AppResult<Option<OrderDto>> {
        let result = sqlx::query(r#"SELECT * FROM orders WHERE order_id = $1"#)
            .bind(order_id)
            .fetch_optional(&self.pool)
            .await
    ...
    pub async fn list_orders(&self, query: OrderQuery) -> AppResult<PagedResult<OrderDto>> {
        ...
        if let Some(order_no) = &query.order_no {
            if !order_no.is_empty() {
                // 安全验证：限制 LIKE 模式的长度
                let safe_no = order_no.chars().take(50).collect::<String>();
                conditions.push(format!("order_no LIKE ${}", params.len() + 1));
                params.push(format!("%{}%", safe_no));
            }
        }
        ...
        if let Some(vehicle_id) = query.vehicle_id {
            // 安全验证：确保是有效数字
            if vehicle_id > 0 {
                conditions.push(format!("vehicle_id = ${}", params.len() + 1));
        ...
        if let Some(customer_name) = &query.customer_name {
            if !customer_name.is_empty() {
                // 安全验证：限制 LIKE 模式的长度
                let safe_name = customer_name.chars().take(100).collect::<String>();
                conditions.push(format!("customer_name LIKE ${}", params.len() + 1));
        ...
        if let Some(order_status) = query.order_status {
            // 安全验证：确保是有效数字
            if order_status >= 0 {
                conditions.push(format!("order_status = ${}", params.len() + 1));
    ...
    pub async fn get_order_items(&self, order_id: i32) -> AppResult<Vec<OrderItemDto>> {
        let rows = sqlx::query(r#"SELECT * FROM order_items WHERE order_id = $1 ORDER BY item_id"#)
            .bind(order_id)
            .fetch_all(&self.pool)
            .await
    ...
    pub async fn delete_order_item(&self, item_id: i32) -> AppResult<bool> {
        let result = sqlx::query(r#"DELETE FROM order_items WHERE item_id = $1"#)
            ...
            .await
            .map_err(|e| {
                AppError::db_error(&format!("Failed to delete order item: {}", e), None)
            })?;
    ...
    pub async fn create_logistics_track(
        ...
        .await
        .map_err(|e| {
            AppError::db_error(&format!("Failed to create logistics track: {}", e), None)
        })?;
        ...
        sqlx::query(r#"UPDATE orders SET order_status = $1 WHERE order_id = $2"#)
            ...
            .await
            .map_err(|e| {
                AppError::db_error(&format!("Failed to update order status: {}", e), None)
            })?;
        ...
        Ok(crate::models::LogisticsTrack {
            ...
            track_time: row
                .try_get::<Option<chrono::DateTime<chrono::Utc>>, _>("track_time")
                .ok()
                .flatten(),
            ...
            create_time: row
                .try_get::<Option<chrono::DateTime<chrono::Utc>>, _>("create_time")
                .ok()
                .flatten()
    ...
    pub async fn update_logistics_track(
        ...
        .await
        .map_err(|e| {
            AppError::db_error(&format!("Failed to update logistics track: {}", e), None)
        })?;
    ...
    pub async fn delete_logistics_track(&self, track_id: i32) -> AppResult<bool> {
        let result = sqlx::query(r#"DELETE FROM logistics_tracks WHERE track_id = $1"#)
            ...
            .await
            .map_err(|e| {
                AppError::db_error(&format!("Failed to delete logistics track: {}", e), None)
            })?;
    ...
    pub async fn get_vehicle_tracks(
        ...
        Ok(crate::application::PagedResult::new(
    ...
    pub async fn get_driver_name(&self, driver_id: i32) -> AppResult<Option<String>> {
        ...
        Ok(row.and_then(|r| {
            r.try_get::<Option<String>, _>("driver_name")
                .unwrap_or(None)
        }))
    }
    ...
    fn row_to_dto(&self, row: &sqlx::postgres::PgRow) -> OrderDto {
        OrderDto {
            ...
            create_time: row
                .try_get::<chrono::DateTime<chrono::Utc>, _>("create_time")
                .map(|t| t.naive_utc())
                .unwrap_or_else(|_| chrono::Utc::now().naive_utc()),
            update_time: row
                .try_get::<Option<chrono::DateTime<chrono::Utc>>, _>("update_time")
                .map(|t| t.map(|t| t.naive_utc()))
                .unwrap_or(None),
        }
    }
}
