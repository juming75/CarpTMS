...
use crate::domain::entities::location::{Fence, FenceCreate, FenceQuery, FenceUpdate, Location, LocationCreate, LocationUpdate, Place, PlaceCreate, PlaceUpdate, Route, RouteCreate, RouteUpdate};
...
pub trait LocationService: Send + Sync {
    ...
    async fn update_location(&self, location_id: i32, location: LocationUpdate) -> AppResult<Option<Location>>;
...
impl LocationService for LocationServiceImpl {
    ...
    // ==================== 位置相关 ====================
    async fn get_locations(&self, page: i32, page_size: i32) -> AppResult<(Vec<Location>, i64)> {
        self.location_repository.get_locations(page, page_size).await
    }
    ...
    async fn update_location(&self, location_id: i32, location: LocationUpdate) -> AppResult<Option<Location>> {
        self.location_repository.update_location(location_id, location).await
    }
