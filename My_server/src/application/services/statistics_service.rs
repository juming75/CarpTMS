...
use crate::domain::entities::statistics::{
    CustomStatistics, DeviceStatistics, SafetyRankingItem, StatisticsItem, VehicleStatistics,
    WeighingStatistics,
};
...
impl StatisticsService {
    ...
    pub async fn get_vehicle_statistics(&self) -> AppResult<VehicleStatistics> {
        ...
        let total_vehicles: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM vehicles WHERE vehicle_type IS NOT NULL")
                .fetch_one(&self.pool)
                .await
                .unwrap_or(0);
    ...
    pub async fn get_device_statistics(&self) -> AppResult<DeviceStatistics> {
        ...
        // 绑定车辆的设备数
        let devices_with_vehicles: i64 =
            sqlx::query_scalar("SELECT COUNT(*) FROM devices WHERE vehicle_id IS NOT NULL")
                .fetch_one(&self.pool)
                .await
                .unwrap_or(0);
    ...
    pub async fn get_weighing_statistics(
        ...
        let total_weight: f64 = sqlx::query_scalar(
            "SELECT COALESCE(SUM(gross_weight - tare_weight), 0) FROM weighing_records",
        )
        .fetch_one(&self.pool)
        .await
        .unwrap_or(0.0);
