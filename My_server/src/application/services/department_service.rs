...
use crate::domain::entities::department::{Department, DepartmentCreateRequest, DepartmentUpdateRequest};
...
impl DepartmentService {
    ...
    /// 获取部门列表
    pub async fn get_departments(&self, page: i32, page_size: i32) -> AppResult<(Vec<Department>, i64)> {
        ...
        let total: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM departments")
            .fetch_one(&self.pool)
            .await
            .map_err(|e| AppError::db_error(&format!("Failed to count departments: {}", e), None))?;

        // 查询列表
        let rows = sqlx::query(
            r#"SELECT d.*, 
               COALESCE(p.department_name, '') as parent_department_name, 
               COALESCE(u.user_name, '') as manager_name
        FROM departments d 
        ...
        let departments: Vec<Department> = rows.iter().map(|r| {
    ...
    /// 创建部门
    pub async fn create_department(&self, request: DepartmentCreateRequest) -> AppResult<Department> {
        // 检查父部门是否存在
        if let Some(parent_id) = request.parent_department_id {
            let exists: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM departments WHERE department_id = $1")
                .bind(parent_id)
                .fetch_one(&self.pool)
                .await
                .map_err(|e| AppError::db_error(&format!("Failed to check parent: {}", e), None))?;
            
            if exists == 0 {
                return Err(AppError::business_error("Parent department not found", None));
            }
        }
    ...
    pub async fn update_department(
        ...
        let exists: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM departments WHERE department_id = $1")
            ...
            .fetch_one(&self.pool)
            .await
            .map_err(|e| AppError::db_error(&format!("Failed to check department: {}", e), None))?;

        if exists == 0 {
            return Err(AppError::not_found_error("Department not found".to_string()));
        }

        // 检查父部门是否存在且不是自己
        if let Some(parent_id) = request.parent_department_id {
            if parent_id == department_id {
                return Err(AppError::business_error("Department cannot be its own parent", None));
            }
            let parent_exists: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM departments WHERE department_id = $1")
                .bind(parent_id)
                .fetch_one(&self.pool)
                .await
                .map_err(|e| AppError::db_error(&format!("Failed to check parent: {}", e), None))?;
    ...
    pub async fn delete_department(&self, department_id: i32) -> AppResult<()> {
        ...
        // 检查是否有子部门
        let child_count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM departments WHERE parent_department_id = $1")
            .bind(department_id)
            .fetch_one(&self.pool)
            .await
            .map_err(|e| AppError::db_error(&format!("Failed to check children: {}", e), None))?;

        if child_count > 0 {
            return Err(AppError::business_error("Cannot delete department with sub-departments", None));
        }
        ...
        sqlx::query("DELETE FROM departments WHERE department_id = $1")
            ...
            .execute(&self.pool)
            .await
            .map_err(|e| AppError::db_error(&format!("Failed to delete department: {}", e), None))?;
