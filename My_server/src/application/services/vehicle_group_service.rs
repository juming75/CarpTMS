...
impl VehicleGroupService {
    ...
    pub async fn get_vehicle_groups(
        ...
        page: i32,
        page_size: i32,
    ) -> Result<(Vec<VehicleGroup>, i64), AppError> {
        self.repository
            .find_all(page, page_size)
            .await
            .map_err(|e| AppError::internal_error("Database error", Some(&e.to_string())))
    }
    ...
    pub async fn create_vehicle_group(
        &self,
        request: VehicleGroupCreateRequest,
    ) -> Result<VehicleGroup, AppError> {
        // 检查车组名称是否已存在
        let existing_count = self
            .repository
            .count_by_name(&request.group_name, None)
            .await
            .map_err(|e| AppError::internal_error("Database error", Some(&e.to_string())))?;
        if existing_count > 0 {
            return Err(AppError::business_error(
                "Vehicle group name already exists",
                None,
            ));
        }
        ...
        if let Some(ref group_name) = request.group_name {
            if !group_name.is_empty() {
                let duplicate_count = self
                    ...
                    .await
                    .map_err(|e| {
                        AppError::internal_error("Database error", Some(&e.to_string()))
                    })?;
