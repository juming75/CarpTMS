...
use crate::infrastructure::repositories::calibration_repository::{CalibrationRepository, PgCalibrationRepository};
...
impl CalibrationServiceImpl {
    ...
    fn fit_polynomial(
        ...
        let mut math_svc = crate::services::weight_calibration_service::WeightCalibrationService::new();
        ...
        for p in &pts_ref {
            let y_pred = crate::services::weight_calibration_service::PolynomialCoefficients::calculate(
                &coeffs.after_turning, p.pa_value as f64,
            );
...
impl CalibrationService for CalibrationServiceImpl {
    async fn get_calibrations(
        ...
        info!("Fetched {} calibrations out of total {}", calibrations.len(), total);
    ...
    async fn create_calibration(&self, calibration: &SensorCalibration) -> Result<SensorCalibration, AppError> {
        ...
        let (polynomial_json, r2_score, rmse, max_error, point_count) =
            if let Some(pts_json) = &calibration.calibration_points {
                self.fit_polynomial(pts_json, &calibration.plate_no, 1, true, 50000.0)
                    .unwrap_or_else(|| {
                        (calibration.polynomial_json.clone().unwrap_or_default(), 0.0, 0.0, 0.0, 0)
                    })
            } else {
                (
        ...
        let _ = self.calibration_repository.create_calibration_history(&history).await;

        info!("Created calibration for sensor: {} (R²={:.4})", calibration.sensor_no, r2_score);
    ...
    async fn update_calibration(&self, id: i32, calibration: &SensorCalibration) -> Result<SensorCalibration, AppError> {
        ...
        // 检查标定数据是否存在
        let existing_calibration: Option<SensorCalibration> = self.calibration_repository.get_calibration(id).await?;
    ...
    async fn get_calibration_history(
        ...
        info!("Fetched {} calibration history records out of total {}", history.len(), total);
        Ok((history, total))
    }
    ...
    fn validate_calibration_data(&self, calibration: &SensorCalibration) -> Result<(), AppError> {
        ...
        // 如果已标定，验证多项式系数
        if calibration.is_calibrated
            && (calibration.polynomial_json.is_none() || calibration.polynomial_json.as_deref().is_none_or(|s| s.is_empty())) {
                return Err(AppError::business_error(
