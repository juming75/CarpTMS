...
fn row_to_role_response(row: &sqlx::postgres::PgRow) -> RoleResponse {
    RoleResponse {
        ...
        created_at: row
            ...
            .unwrap_or_default(),
        updated_at: row
            .get::<Option<chrono::NaiveDateTime>, _>(4)
            .map(|t| t.to_string())
            .unwrap_or_default(),
    }
}
...
impl RoleApplicationService {
    ...
    pub async fn get_roles(&self) -> AppResult<Vec<RoleResponse>> {
        let rows = sqlx::query(
            "SELECT group_id, group_name, description, create_time, update_time FROM user_groups",
        )
        .fetch_all(&self.pool)
        .await?;
    ...
    pub async fn get_role(&self, role_id: i32) -> AppResult<Option<RoleResponse>> {
        let row = sqlx::query(
            "SELECT group_id, group_name, description, create_time, update_time FROM user_groups WHERE group_id = $1",
        )
        .bind(role_id)
    ...
    pub async fn create_role(&self, request: RoleCreateRequest) -> AppResult<RoleResponse> {
        ...
        let row = sqlx::query(
            "INSERT INTO user_groups (group_name, description, create_time, update_time)
             VALUES ($1, $2, $3, $4)
             RETURNING group_id, group_name, description, create_time, update_time",
        )
    ...
    pub async fn update_role(
        ...
        let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM user_groups WHERE group_id = $1")
            .bind(role_id)
            .fetch_one(&self.pool)
            .await?;
