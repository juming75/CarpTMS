...
impl AuditLogger {
    ...
    pub async fn log(
        ...
        if self.should_log(&level) {
            ...
            if let Some(file) = log_file.as_mut() {
                let log_entry = serde_json::to_string(&entry).expect("审计日志序列化失败");
    ...
    fn should_log(&self, level: &AuditLogLevel) -> bool {
        match (&self.log_level, level) {
            ...
            (
                AuditLogLevel::Warning,
                AuditLogLevel::Warning | AuditLogLevel::Error | AuditLogLevel::Critical,
            ) => true,
    ...
    fn log_to_console(&self, entry: &AuditLogEntry) {
        ...
        println!(
            "[{}] [{}]{}{} {}",
            timestamp, level_str, user_str, ip_str, entry.message
        );
