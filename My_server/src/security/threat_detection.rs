...
impl ThreatDetector {
    ...
    async fn detect_brute_force(&self, threats: &mut Vec<ThreatEvent>) {
        ...
        for (ip, records) in &*ip_access_records {
            // 统计最近5分钟内的登录失败次数
            let now = Instant::now();
            let login_failures = records
                .iter()
                .filter(|record| {
                    record.path == "/api/auth/login"
                        && record.status_code == 401
                        && now.duration_since(record.timestamp) < Duration::from_secs(5 * 60)
                })
                .count();
    ...
    /// 检测SQL注入
    async fn detect_sql_injection(&self, _threats: &mut Vec<ThreatEvent>) {
        // 这里可以添加SQL注入检测逻辑
        // 例如检查请求参数中是否包含SQL注入攻击特征
    }

    /// 检测XSS
    async fn detect_xss(&self, _threats: &mut Vec<ThreatEvent>) {
        // 这里可以添加XSS检测逻辑
        // 例如检查请求参数中是否包含XSS攻击特征
    }

    /// 检测命令注入
    async fn detect_command_injection(&self, _threats: &mut Vec<ThreatEvent>) {
        // 这里可以添加命令注入检测逻辑
    ...
    async fn detect_sensitive_information_access(&self, threats: &mut Vec<ThreatEvent>) {
        ...
        for (ip, records) in &*ip_access_records {
            // 统计最近10分钟内的敏感路径访问次数
            let now = Instant::now();
            let sensitive_accesses = records
                .iter()
                .filter(|record| {
                    self.sensitive_paths.contains(&record.path)
                        && now.duration_since(record.timestamp) < Duration::from_secs(10 * 60)
                })
                .count();
    ...
    async fn detect_abnormal_access_pattern(&self, threats: &mut Vec<ThreatEvent>) {
        ...
        for (ip, records) in &*ip_access_records {
            // 检查是否存在快速连续访问
            let now = Instant::now();
            let recent_accesses = records
                .iter()
                .filter(|record| now.duration_since(record.timestamp) < Duration::from_secs(10))
                .count();
