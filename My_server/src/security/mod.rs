...
pub use audit_log::{AuditLogger, AuditLogEntry, AuditLogLevel};
...
impl SecurityManager {
    ...
    /// 记录审计日志
    pub async fn log_audit(&self, level: AuditLogLevel, message: &str, details: Option<serde_json::Value>) {
    ...
    pub async fn run_security_assessment(&self) -> Result<serde_json::Value, anyhow::Error> {
        self.log_audit(AuditLogLevel::Info, "开始安全评估", None).await;
        
        let report = self.generate_security_report().await?;
        
        self.log_audit(AuditLogLevel::Info, "安全评估完成", Some(report.clone())).await;
