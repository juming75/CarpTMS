...
impl BackupManager {
    ...
    pub async fn restore_from_backup(&self, backup_id: &str) -> Result<(), DisasterRecoveryError> {
        ...
        // 实际恢复操作由 storage_backend.retrieve() 完成，
        // 数据已通过上面的 retrieve → decrypt → decompress → verify 加载到 backup_data
        info!(
            "Restoration from backup {} completed: {} bytes restored and verified",
            backup_id,
            backup_data.len(),
        );
        Ok(())
    ...
    /// 生成备份数据：序列化内存中的活跃备份元数据为 JSON
    async fn generate_backup_data(
        &self,
        backup_type: BackupType,
    ) -> Result<Vec<u8>, DisasterRecoveryError> {
        let backups = self.active_backups.read().await;
        let mut snapshot = serde_json::json!({
            "backup_type": format!("{}", backup_type),
            "created_at": chrono::Utc::now().to_rfc3339(),
            "app_version": "2.0.0",
            "metadata_entries": &*backups,
        });
        // 兼容旧逻辑：保留 data 字段
        if let Some(obj) = snapshot.as_object_mut() {
            obj.insert("data_size".into(), serde_json::json!(backups.len()));
        }
        serde_json::to_vec(&snapshot)
            .map_err(|e| DisasterRecoveryError::BackupFailed(e.to_string()))
    }
    ...
    pub async fn trigger_failover(&self, reason: String) -> Result<(), DisasterRecoveryError> {
        ...
        *self.failover_status.write().await = Some(status);
        ...
        tokio::spawn({
            ...
            let endpoints = self.config.replication_endpoints.clone();

            async move {
                info!("Failover: probing standby endpoints...");
                for endpoint in &endpoints {
                    let client = reqwest::Client::builder()
                        .timeout(Duration::from_secs(10))
                        .build();
                    match client {
                        Ok(c) => match c.get(format!("{}/health", endpoint)).send().await {
                            Ok(_) => info!("Failover: endpoint {} reachable", endpoint),
                            Err(e) => warn!("Failover: endpoint {} unreachable: {}", endpoint, e),
                        },
                        Err(e) => warn!("Failover: failed to create HTTP client: {}", e),
                    }
                }
...
impl ConsistencyChecker {
    ...
    async fn check_database_consistency(&self) -> Result<bool, DisasterRecoveryError> {
        ...
        match &self.pool {
            Some(pool) => match sqlx::query("SELECT 1").execute(pool).await {
                ...
                Err(e) => {
                    warn!("Database connection issue during consistency check: {}", e);
                    Ok(false)
                }
            },
            None => {
                info!("Database pool not available, skipping DB consistency check");
                Ok(true)
            }
        }
    }
    ...
    async fn check_file_system_consistency(&self) -> Result<bool, DisasterRecoveryError> {
        let path = &self.config.backup_storage_path;
        match tokio::fs::metadata(path).await {
            Ok(_) => {
                info!("Backup storage path exists: {}", path.display());
                Ok(true)
            }
            Err(e) => {
                warn!("Backup storage path issue: {} - {}", path.display(), e);
                Ok(false)
            }
        }
    }
    ...
    async fn check_replication_consistency(&self) -> Result<bool, DisasterRecoveryError> {
        ...
        for endpoint in &self.config.replication_endpoints {
            ...
            if ok {
                info!("Replication endpoint {} is healthy", endpoint);
            } else {
                warn!("Replication endpoint {} is unreachable", endpoint);
                all_ok = false;
            }
        }
        Ok(all_ok)
