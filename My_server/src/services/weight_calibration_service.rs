...
impl WeightCalibrationService {
    ...
    pub fn create_calibration_sheet(
        ...
        self.calibration_sheets.insert(plate_number.to_string(), sheet);
        info!(
            "创建车辆标定表: {}, 轴数: {}, 额定总重: {}kg",
    ...
    pub fn add_calibration_point(
        &mut self,
        params: CalibrationPointParams,
    ) -> Result<(), String> {
        let sheet = self.calibration_sheets.get_mut(&params.plate_number)
            .ok_or_else(|| format!("标定表不存在: {}", params.plate_number))?;

        let axle_record = sheet.axle_records.get_mut(&params.axle_number)
        ...
        if let Some(existing) = wheel_record.calibration_points.iter_mut()
    ...
    pub fn add_calibration_point_with_time(
        ...
        if let Some(existing) = wheel_record.calibration_points.iter_mut()
            ...
            info!(
                ...
                params.plate_number, params.axle_number, params.load_percentage, params.pa_raw, pa_value
            );
        } else {
            let point = CalibrationPoint {
                sensor_address,
    ...
    pub fn calculate_sensor_coefficients(
        ...
        let axle_record = sheet.axle_records.get(&axle_number)
        ...
        // 分割为转折前和转折后两组
        let before_turning: Vec<_> = points.iter()
            .filter(|p| (p.pa_value as f64) < turning_point)
            .cloned()
            .collect();

        let after_turning: Vec<_> = points.iter()
            .filter(|p| (p.pa_value as f64) >= turning_point)
            .cloned()
            .collect();
        ...
        // 更新到标定表
        let sheet = self.calibration_sheets.get_mut(plate_number)
            .ok_or_else(|| format!("标定表不存在: {}", plate_number))?;
        let axle = sheet.axle_records.get_mut(&axle_number)
            .ok_or_else(|| format!("轴号不存在: {}", axle_number))?;
    ...
    pub fn calculate_first_order_coefficients(
        ...
        let sum_xy: f64 = points.iter().map(|p| p.pa_value as f64 * p.actual_weight).sum();
    ...
    pub fn calculate_second_order_coefficients(
        ...
        let sum_y: f64 = points.iter().map(|p| p.actual_weight).sum();
        let sum_xy: f64 = points.iter().map(|p| p.pa_value as f64 * p.actual_weight).sum();
        let sum_x2y: f64 = points.iter().map(|p| (p.pa_value as f64).powi(2) * p.actual_weight).sum();
        ...
        let a_mat_c = [[b_vec[0], a_mat[0][1], a_mat[0][2]], [b_vec[1], a_mat[1][1], a_mat[1][2]], [b_vec[2], a_mat[2][1], a_mat[2][2]]];
        let a_mat_b = [[a_mat[0][0], b_vec[0], a_mat[0][2]], [a_mat[1][0], b_vec[1], a_mat[1][2]], [a_mat[2][0], b_vec[2], a_mat[2][2]]];
        let a_mat_a = [[a_mat[0][0], a_mat[0][1], b_vec[0]], [a_mat[1][0], a_mat[1][1], b_vec[1]], [a_mat[2][0], a_mat[2][1], b_vec[2]]];
    ...
    pub fn calculate_sensor_weight(
    ...
    pub fn calculate_load_weight(total_weight: f64, tare_weight: f64) -> f64 {
        ...
        info!("计算载重: {:.2}kg (总重{:.2}kg - 自重{:.2}kg)", load, total_weight, tare_weight);
        load
    }
            ...
            if let Some(axle) = sheet.axle_records.get(&axle_num) {
                ...
                if axle.left_wheel.calibration_points.len() >= order.required_points() {
                    let coeffs = Self::calculate_coefficients(
                        &axle.left_wheel.calibration_points,
                        order,
                    )?;
                ...
                if axle.right_wheel.calibration_points.len() >= order.required_points() {
                    let coeffs = Self::calculate_coefficients(
                        &axle.right_wheel.calibration_points,
                        order,
                    )?;
    ...
    pub fn fetch_pa_value_at_time(
        ...
        // 在容忍时间范围内查找最接近的数据点
        let closest = sensor_history.iter().min_by_key(|(time, _)| {
            (*time - target_time).num_seconds().abs()
        });
    ...
    pub fn create_calibration_table(
        ...
        info!("创建车辆标定表: {}, 额定总重: {}kg", plate_number, rated_total_weight);
        table
    }
...
mod tests {
    ...
    fn test_calibration_sheet() {
        ...
        // 创建2轴车辆标定表
        let sheet = service.create_calibration_sheet(
            "川J53757",
            32000.0,  // 额定总重32吨
            12000.0,  // 空车12吨
            2,        // 2轴
        ...
        service.add_calibration_point(CalibrationPointParams {
        ...
        // 生成一阶系数
        let coeffs = service.generate_vehicle_coefficients("川J53757", PolynomialOrder::First).unwrap();
        println!("生成的一阶系数: {:?}", coeffs);
        assert_eq!(coeffs.len(), 2); // 第一轴左右轮

        // 获取传感器地址
        let left_addr = service.get_calibration_sheet("川J53757")
            .and_then(|s| s.axle_records.get(&1))
            .map(|a| a.left_wheel.sensor_address)
            .unwrap();
        let right_addr = service.get_calibration_sheet("川J53757")
            .and_then(|s| s.axle_records.get(&1))
        ...
        let total_weight = WeightCalibrationService::calculate_total_weight(&sensor_coeffs, &sensor_pa);
