...
use crate::config::{UnifiedConfig, ArchitectureMode, MicroserviceConfig, BoundedContextConfig};
...
impl MicroDDDBootstrap {
    ...
    fn get_current_service<'a>(&self, config: &'a UnifiedConfig) -> Option<&'a MicroserviceConfig> {
        ...
        config.architecture.microservices.iter()
            .find(|s| &s.name == name)
    }

    /// 获取当前服务的限界上下文
    fn get_bounded_context<'a>(&self, config: &'a UnifiedConfig) -> Option<&'a BoundedContextConfig> {
        let service_name = self.service_name.as_ref()
            .or(config.architecture.service_name.as_ref())?;
        ...
        config.architecture.bounded_contexts.iter()
    ...
    /// 初始化领域层
    async fn initialize_domain_layer(
        &self,
    ...
    /// 初始化事件驱动架构
    async fn initialize_event_system(
        &self,
        config: &UnifiedConfig,
    ) -> AppResult<()> {
    ...
    async fn init_internal(&self, config: &UnifiedConfig) -> AppResult<ApplicationState> {
        ...
        // 1. 验证服务配置
        let service_config = self.get_current_service(config)
        ...
        let pool = Arc::new(
            config.get_pool_config()
                .max_connections(config.architecture.adjust_db_config(config.database.max_connections))
    ...
    async fn start_internal(&self, config: UnifiedConfig) -> AppResult<()> {
        ...
        let port = config.architecture.current_service_port(service_config.port);
...
mod tests {
    ...
    fn test_service_to_context_mapping() {
        ...
        assert_eq!(bootstrap.service_to_context_name("vehicle-service"), "transportation");
        assert_eq!(bootstrap.service_to_context_name("trip-service"), "transportation");
        assert_eq!(bootstrap.service_to_context_name("cargo-service"), "cargo");
        assert_eq!(bootstrap.service_to_context_name("billing-service"), "billing");
    }
}
