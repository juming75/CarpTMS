...
pub fn start_sync_service(pool: Arc<sqlx::PgPool>) {
    tokio::spawn(async move {
        ...
        // 从环境变量加载同步配置
        let legacy_config = tms_server::sync::config::LegacyServerConfig {
        ...
        let sync_config = tms_server::sync::config::SyncConfig {
            enabled: std::env::var("LEGACY_SYNC_ENABLED")
                .ok()
                .and_then(|s| s.parse::<bool>().ok())
                .unwrap_or(true),
        ...
        // 创建适配器
        let adapter = tms_server::sync::adapter::LegacySyncAdapter::new(legacy_config);

        // 创建同步服务
        let sync_service =
            tms_server::sync::service::DataSyncService::new(adapter, pool, sync_config);
