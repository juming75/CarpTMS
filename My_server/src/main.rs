...
use carptms::init;
use carptms::logging::init_log_manager;

use carptms::infrastructure::monitoring::{default_switching_config, MonitoringManager};
use carptms::telemetry::{init_telemetry, shutdown_telemetry};
use carptms::tracing::{init_tracing, shutdown_tracing};

/// 主函数 - 启动CarpTMS服务器
#[actix_rt::main]
async fn main() -> std::io::Result<()> {
    // 1. 初始化追踪和日志
    let tracing_config = carptms::tracing::TracingConfig::default();
    let _ = init_tracing(&tracing_config);
    let _ = init_telemetry("carptms-server");
    let _ = init_log_manager(10000);

    // 初始化监控指标
    carptms::metrics::init_metrics();

    // 初始化内存监控和限制
    let memory_monitor_config = carptms::performance::MemoryMonitorConfig {
        memory_threshold: 90.0,
        check_interval: std::time::Duration::from_secs(60),
        memory_limit: Some(32 * 1024 * 1024 * 1024),
        enable_memory_limit: false,
    };

    let memory_limit_service =
        carptms::performance::MemoryLimitService::new(memory_monitor_config, None);
    memory_limit_service.start();
    tracing::info!("Memory limit service started with 16GB limit");

    // 初始化资源告警服务
    let resource_alert_config = carptms::performance::ResourceAlertConfig {
        cpu_threshold: 80.0,
    ...
    let resource_alert_service =
        carptms::performance::ResourceAlertService::new(resource_alert_config);
    ...
    // 2. 加载配置并初始化
    let (app_config, redis_client) = init::init_all()
        .await
    ...
    // 5. 检查JWT密钥配置
    if let Err(e) = carptms::utils::jwt::check_jwt_secret() {
...
async fn start_api_gateway(
    server_state: carptms::init::finalize::ServerState,
    redis_client: Arc<::redis::Client>,
) -> std::io::Result<()> {
    let config = &server_state.config;
    let server_addr = format!("{}:{}", config.server.host, config.server.port);
    let protocol = if config.security.enable_https {
        "https"
    } else {
        "http"
    };
    ...
    let circuit_breaker = std::sync::Arc::new(carptms::errors::CircuitBreaker::with_defaults(
        "api-gateway",
    ));

    // Create API Gateway config
    let gateway_config = carptms::gateway::api_gateway::ApiGatewayConfig {
        address: server_addr,
        server_state,
    ...
    let api_gateway = carptms::gateway::api_gateway::ApiGateway::new(gateway_config);
