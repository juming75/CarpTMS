...
impl ServiceRegistry {
    ...
    pub async fn get_services_by_name(&self, service_name: &str) -> Vec<ServiceInfo> {
        if let (Ok(name_index), Ok(services)) =
            (self.service_name_index.read(), self.services.read())
        {
    ...
    /// 根据服务名称和版本获取服务
    pub async fn get_services_by_version(
        &self,
        service_name: &str,
        version: &str,
    ) -> Vec<ServiceInfo> {
    ...
    /// 设置服务元数据
    pub async fn set_service_metadata(
        &self,
        service_id: &str,
        key: &str,
        value: &str,
    ...
    /// 获取服务的所有元数据
    pub async fn get_service_all_metadata(
        &self,
        service_id: &str,
    ...
    pub async fn remove_service_dependency(
        ...
        if let Some(service_deps) = dependencies.get_mut(service_id) {
            ...
            log::debug!(
                "Removed dependency {} for service {}",
                dependency_id,
                service_id
            );
            Ok(())
    ...
    /// 负载均衡选择服务（基于版本）
    pub async fn select_service_by_version(
        &self,
        service_name: &str,
        version: &str,
    ) -> Option<ServiceInfo> {
