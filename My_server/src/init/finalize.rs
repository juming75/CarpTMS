...
use crate::ai::qwen3_5::gguf_loader::Qwen3_5GGufLoader;
use crate::ai::qwen3_5::pipeline::Qwen3_5Pipeline;
...
use crate::domain::use_cases::auth::{AuthRepository, AuthUseCases};
use crate::domain::use_cases::department::{DepartmentRepository, DepartmentUseCases};
...
use crate::domain::use_cases::openapi_platform::{
    OpenapiPlatformRepository, OpenapiPlatformUseCases,
};
use crate::domain::use_cases::organization::{OrganizationRepository, OrganizationUseCases};
use crate::domain::use_cases::statistic::{StatisticRepository, StatisticUseCases};
...
pub struct ServerState {
    ...
    pub openapi_platform_service:
        Arc<crate::domain::use_cases::openapi_platform::OpenapiPlatformUseCases>,
    pub system_monitor_service:
        Arc<crate::application::services::system_monitor_service::SystemMonitorService>,
    ...
    pub ml_service: Arc<crate::ml::MachineLearningService>,
    pub calibration_service: Arc<crate::application::services::calibration_service::CalibrationServiceImpl>,
    pub weighing_data_service: Arc<crate::application::services::weighing_data_service::WeighingDataServiceImpl>,
}
...
pub async fn finalize_setup(
    ...
    // 执行数据库迁移（幂等安全，不影响已迁移的库）
    if let Err(e) = crate::database::migration::run_migrations(&pool).await {
        warn!("数据库迁移执行不完整 ({}), 服务继续以现有 Schema 运行", e);
    }
    ...
    // 注册 Qwen3.5 GatedDeltaNet 架构
    Qwen3_5GGufLoader::register();

    // 初始化AI服务（Qwen3.5纯本地推理）
    let ai_service_state = init_ai_service();

    // 初始化司机服务
    let driver_repository: Arc<dyn DriverRepository + Send + Sync> =
        Arc::new(PgDriverRepository::new(Arc::new(pool.clone())));
    let driver_service = Arc::new(DriverUseCases::new(driver_repository));

    // 初始化部门服务
    let department_repository: Arc<dyn DepartmentRepository + Send + Sync> =
        Arc::new(DepartmentRepositoryImpl::new(Arc::new(pool.clone())));
    let department_service = Arc::new(DepartmentUseCases::new(department_repository));

    // 初始化认证服务
    let auth_repository: Arc<dyn AuthRepository + Send + Sync> =
        Arc::new(AuthRepositoryImpl::new(Arc::new(pool.clone())));
    let auth_service = Arc::new(AuthUseCases::new(auth_repository));

    // 初始化组织单位服务
    let organization_repository: Arc<dyn OrganizationRepository + Send + Sync> =
        Arc::new(OrganizationRepositoryImpl::new(Arc::new(pool.clone())));
    let organization_service = Arc::new(OrganizationUseCases::new(organization_repository));

    // 初始化统计分析服务
    let statistic_repository: Arc<dyn StatisticRepository + Send + Sync> =
        Arc::new(StatisticRepositoryImpl::new(Arc::new(pool.clone())));
    let statistic_service = Arc::new(StatisticUseCases::new(statistic_repository));

    // 初始化车辆服务
    let vehicle_repository: Arc<dyn VehicleRepository + Send + Sync> =
        Arc::new(PgVehicleRepository::new(Arc::new(pool.clone())));
    let vehicle_service = Arc::new(VehicleUseCases::new(vehicle_repository));

    // 初始化位置服务
    let location_repository: Arc<dyn LocationRepository + Send + Sync> =
        Arc::new(PgLocationRepository::new(Arc::new(pool.clone())));
    ...
    let user_repository: Arc<dyn UserRepository + Send + Sync> =
        Arc::new(PgUserRepository::new(Arc::new(pool.clone())));
    let user_use_cases = Arc::new(UserUseCases::new(user_repository));
    let user_service = Arc::new(UserServiceImpl::new(user_use_cases));

    // 初始化设备服务
    let device_repository: Arc<dyn DeviceRepository + Send + Sync> =
        Arc::new(PgDeviceRepository::new(Arc::new(pool.clone())));
    ...
    // 初始化同步服务
    let sync_repository: Arc<dyn SyncRepository + Send + Sync> =
        Arc::new(SqlxSyncRepository::new(pool.clone()));
    let sync_service = Arc::new(SyncUseCases::new(sync_repository));

    // 初始化订单服务
    let order_service = Arc::new(
        crate::application::services::order_service::OrderApplicationService::new(pool.clone()),
    );

    // 初始化财务管理服务
    let finance_repository: Arc<
        dyn crate::domain::use_cases::finance::FinanceRepository + Send + Sync,
    > = Arc::new(
        crate::infrastructure::repositories::finance_repository::FinanceRepositoryImpl::new(
            Arc::new(pool.clone()),
        ),
    );
    let finance_service: Arc<dyn crate::application::services::finance_service::FinanceService> =
        Arc::new(
            crate::application::services::finance_service::FinanceServiceImpl::new(
                finance_repository,
            ),
        );
    ...
    // 初始化车队服务
    let vehicle_group_repository: Arc<dyn VehicleGroupRepository + Send + Sync> =
        Arc::new(PgVehicleGroupRepository::new(Arc::new(pool.clone())));
    let vehicle_group_service = Arc::new(VehicleGroupUseCases::new(vehicle_group_repository));

    // 初始化 OpenAPI 平台服务
    let openapi_platform_repository: Arc<dyn OpenapiPlatformRepository + Send + Sync> =
        Arc::new(OpenapiPlatformRepositoryImpl::new(Arc::new(pool.clone())));
    let openapi_platform_service =
        Arc::new(OpenapiPlatformUseCases::new(openapi_platform_repository));

    // 初始化系统监控服务
    let system_monitor_config =
        crate::application::services::system_monitor_service::SystemMonitorConfig {
            check_interval: 10,
            max_processes: 100,
            enable_details: true,
        };
    let system_monitor_service = Arc::new(
        crate::application::services::system_monitor_service::SystemMonitorService::new(
    ...
    // 初始化审计日志服务
    let audit_log_repository =
        crate::infrastructure::repositories::audit_log_repository::AuditLogRepository::new(
            Arc::new(pool.clone()),
        );
    let audit_log_service = Arc::new(
        crate::application::services::audit_log_service::AuditLogService::new(audit_log_repository),
    );
    ...
    // 初始化机器学习服务
    let ml_service = Arc::new(crate::ml::MachineLearningService::new());

    // 初始化标定服务
    let calibration_repository =
        crate::infrastructure::repositories::calibration_repository::PgCalibrationRepository::new(
            pool.clone(),
        );
    let calibration_service = Arc::new(
        crate::application::services::calibration_service::CalibrationServiceImpl::new(
            calibration_repository,
        ),
    );

    // 初始化称重数据服务
    let weighing_data_repository = Arc::new(
        crate::infrastructure::repositories::weighing_data_repository::PgWeighingDataRepository::new(
            Arc::new(pool.clone()),
        ),
    );
    let event_logger = Arc::new(crate::domain::event_logger::DomainEventLogger::new());
    let weighing_data_service = Arc::new(
        crate::application::services::weighing_data_service::WeighingDataServiceImpl::new(
    ...
    std::thread::spawn(|| {
            ...
            .build()
        {
            Ok(runtime) => runtime.block_on(async {
                match "0.0.0.0:9808".parse::<std::net::SocketAddr>() {
                    Ok(addr) => {
                        let mut server =
                            crate::gateway::client_api_server::ClientApiServer::new(addr, 1000);
                    ...
                    Err(e) => {
                        log::error!("Failed to parse client API server address: {:?}", e);
                    }
                }
            }),
            Err(e) => {
...
/// 初始化 AI 服务（Qwen3.5 Candle 纯本地推理）
fn init_ai_service() -> Arc<crate::ai::routes::AiServiceState> {
    let pipeline = Qwen3_5Pipeline::new();
