    ...
    if video_enabled {
        ...
        match video_service_result {
            ...
            Err(e) => {
                ...
                // 创建一个禁用的视频服务
                Arc::new(
                    crate::video::VideoService::new(crate::video::VideoConfig {
                        enabled: false,
                        ..Default::default()
                    })
                    .expect("Failed to create disabled video service"),
                )
            }
        }
    } else {
...
pub async fn start_disaster_recovery_service(_pool: &PgPool) {
    ...
    if !cfg!(debug_assertions) {
        ...
        use crate::disaster_recovery::{DisasterRecoveryConfig, DisasterRecoveryManager, LocalStorageBackend};
        ...
        // 创建存储后端
        let storage_backend = Arc::new(LocalStorageBackend::new(config.backup_storage_path.clone()));

        // 确保备份目录存在
        if let Err(e) = tokio::fs::create_dir_all(&config.backup_storage_path).await {
            warn!("Failed to create backup directory: {}, disaster recovery may not work properly", e);
        }
        ...
        match DisasterRecoveryManager::new(
            ...
            vec!["secondary-region-1".to_string(), "secondary-region-2".to_string()],
        ).await {
            Ok(dr_manager) => {
                // 启动灾难恢复服务
                tokio::spawn(async move {
