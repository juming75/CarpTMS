...
// 编译期确定的服务地址，消除运行时 parse 可能引发的 panic
const CLIENT_SERVER_ADDR: SocketAddr =
    SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::new(0, 0, 0, 0), 9808));
const VEHICLE_COMM_ADDR: SocketAddr =
    SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::new(0, 0, 0, 0), 8988));
...
pub async fn start_client_server_service() {
    ...
    let server = ClientApiServer::new(CLIENT_SERVER_ADDR, 1000);
...
pub async fn start_vehicle_comm_server(pool: &PgPool, _central_config: &CentralConfig) {
    ...
    actix::spawn(async move {
        let vehicle_comm_server =
            VehicleCommServer::new(VEHICLE_COMM_ADDR, max_connections, Some(pool_clone));
