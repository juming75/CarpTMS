...
impl AdditionalProtocolParser for CanBusParser {
    async fn parse(&self, data: &[u8]) -> Result<Bytes, AdditionalProtocolError> {
        if data.len() < 5 {
            return Err(AdditionalProtocolError::CanBus("Frame too short".to_string()));
        }
...
impl ObdParser {
    pub fn new(ecu_address: u8) -> Self {
        Self { ecu_address }
    }
...
impl AdditionalProtocolParser for ObdParser {
    async fn parse(&self, data: &[u8]) -> Result<Bytes, AdditionalProtocolError> {
        if data.len() < 3 {
            return Err(AdditionalProtocolError::Obd("Message too short".to_string()));
        }
...
impl UdsParser {
    pub fn new(tester_address: u8) -> Self {
        Self { tester_address }
    }
}
...
impl AdditionalProtocolParser for UdsParser {
    async fn parse(&self, data: &[u8]) -> Result<Bytes, AdditionalProtocolError> {
        ...
        let service = match service_id {
            ...
            _ => return Err(AdditionalProtocolError::Uds(format!("Unknown service: 0x{:02X}", service_id))),
        };
...
impl AdditionalProtocolParser for Nmea2000Parser {
    async fn parse(&self, data: &[u8]) -> Result<Bytes, AdditionalProtocolError> {
        if data.len() < 8 {
            return Err(AdditionalProtocolError::Nmea2000("Message too short".to_string()));
        }
        
        // Parse NMEA 2000 message
        let pgn = u32::from_be_bytes([0, data[0], data[1], data[2]]);
        let priority = data[3];
        let source_address = data[4];
        let destination_address = data[5];
...
impl AdditionalProtocolParser for J1939Parser {
    async fn parse(&self, data: &[u8]) -> Result<Bytes, AdditionalProtocolError> {
        if data.len() < 8 {
            return Err(AdditionalProtocolError::J1939("Message too short".to_string()));
        }
...
impl Iso15765Parser {
    pub fn new(block_size: u8, separation_time: u8) -> Self {
        Self { block_size, separation_time }
    }
}
...
impl AdditionalProtocolParser for Iso15765Parser {
    async fn parse(&self, data: &[u8]) -> Result<Bytes, AdditionalProtocolError> {
        ...
        let frame_type = match (frame_type_byte >> 4) & 0x0F {
            0x00 => Iso15765FrameType::SingleFrame(frame_type_byte & 0x0F),
            0x01 => {
                // First frame
                if data.len() < 2 {
                    return Err(AdditionalProtocolError::Iso15765("Invalid first frame".to_string()));
                }
            ...
            0x03 => {
                // Flow control frame
                if data.len() < 3 {
                    return Err(AdditionalProtocolError::Iso15765("Invalid flow control frame".to_string()));
                }
                let block_size = data[1];
                let separation_time = data[2];
                Iso15765FrameType::FlowControlFrame(block_size, separation_time)
            }
            _ => return Err(AdditionalProtocolError::Iso15765("Unknown frame type".to_string())),
        };
    ...
    async fn serialize(&self, message: &Bytes) -> Result<Vec<u8>, AdditionalProtocolError> {
        ...
        // Single frame
        result.push(0x00 | (message.len() as u8 & 0x0F));
...
impl AdditionalProtocolManager {
    ...
    pub async fn new() -> Result<Self, AdditionalProtocolError> {
        ...
        let obd_parser = Arc::new(ObdParser::new(0x7E8)); // ECU address
        let uds_parser = Arc::new(UdsParser::new(0x7E0)); // Tester address
        ...
        let mut parsers: HashMap<AdditionalProtocolType, Arc<dyn AdditionalProtocolParser>> = HashMap::new();
    ...
    pub async fn parse_obd_message(&self, data: &[u8]) -> Result<ObdMessage, AdditionalProtocolError> {
    ...
    /// Parse UDS message
    pub async fn parse_uds_message(&self, data: &[u8]) -> Result<UdsMessage, AdditionalProtocolError> {
    ...
    /// Parse NMEA 2000 message
    pub async fn parse_nmea2000_message(&self, data: &[u8]) -> Result<Nmea2000Message, AdditionalProtocolError> {
    ...
    pub async fn parse_j1939_message(&self, data: &[u8]) -> Result<J1939ParameterGroup, AdditionalProtocolError> {
    ...
    pub async fn parse_iso15765_frame(&self, data: &[u8]) -> Result<Iso15765Frame, AdditionalProtocolError> {
    ...
    pub async fn add_parser(&self, parser: Arc<dyn AdditionalProtocolParser>) -> Result<(), AdditionalProtocolError> {
    ...
    pub async fn remove_parser(&self, protocol_type: &AdditionalProtocolType) -> Result<(), AdditionalProtocolError> {
        let mut parsers = self.parsers.write().await;
