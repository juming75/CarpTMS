...
impl ArchitectureMode {
    ...
    pub fn is_ddd(&self) -> bool {
        matches!(self, ArchitectureMode::MonolithDDD | ArchitectureMode::MicroDDD)
    }
    ...
    pub fn description(&self) -> &'static str {
        match self {
            ArchitectureMode::MonolithDDD => {
                "单体应用架构，所有服务在同一进程，采用领域驱动设计"
            }
            ArchitectureMode::MicroDDD => {
                "微服务架构，服务独立部署，每个服务采用领域驱动设计"
            }
        }
    }
...
impl ArchitectureConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.mode.is_microservice() && self.service_name.is_none() {
            return Err(
                "service_name is required in microservice mode".to_string()
            );
        }
...
fn default_bounded_contexts() -> Vec<BoundedContextConfig> {
    vec![
        BoundedContextConfig {
            ...
            aggregates: vec!["Vehicle".to_string(), "Driver".to_string(), "Trip".to_string()],
