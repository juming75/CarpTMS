...
impl ConfigManager {
    ...
    pub async fn switch_mode(&self, new_mode: ArchitectureMode) -> AppResult<()> {
        ...
        log::info!("Architecture mode switched from {} to {}", old_mode, new_mode);
    ...
    pub async fn validate(&self) -> ValidationResult {
        ...
        // 验证架构模式
        if let Err(e) = config.validate() {
            result.errors.push(format!("Architecture config validation failed: {}", e));
        ...
        if config.mode.is_microservice() {
            if config.service_name.is_none() {
                result.errors.push("service_name is required in microservice mode".to_string());
                result.is_valid = false;
            }
            
            if config.microservices.is_empty() {
                result.warnings.push("No microservices configured".to_string());
            }
        }
        
        // 验证 DDD 配置
        if config.mode.is_ddd()
            && config.bounded_contexts.is_empty() {
                result.warnings.push("No bounded contexts configured for DDD mode".to_string());
            }
    ...
    pub async fn reload_from_env(&self) -> AppResult<()> {
        ...
        let new_mode = if let Ok(mode_str) = std::env::var("ARCHITECTURE_MODE") {
            mode_str.parse().map_err(|e: String| {
                crate::errors::AppError::validation_error(&e, None)
            })?
        } else {
    ...
    pub async fn reload_from_file(&self, path: &str) -> AppResult<()> {
        ...
        let new_config: ArchitectureConfig = toml::from_str(&content)
            .map_err(|e| crate::errors::AppError::validation_error(
...
mod tests {
    ...
    async fn test_switch_mode() {
        ...
        manager.switch_mode(ArchitectureMode::MicroDDD).await.unwrap();
