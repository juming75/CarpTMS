...
fn default_db_url() -> String {
    ...
    std::env::var("DATABASE_URL").unwrap_or_else(|_| {
        "postgresql://postgres:${DB_PASSWORD}@localhost:5432/CarpTMS_db".to_string()
    })
}
...
fn default_jwt_secret() -> String {
    // P2: 从环境变量读取 JWT secret，保留默认值但提示需要修改
    std::env::var("JWT_SECRET")
        .unwrap_or_else(|_| "your-secret-key-change-in-production-123456".to_string())
}
...
fn default_cors_origins() -> Vec<String> {
    // P2: 从环境变量读取 CORS origins，默认包含本地开发地址
...
fn default_jt808_addr() -> String {
    // P2: 从环境变量读取 JT808 地址
    std::env::var("JT808_ADDR").unwrap_or_else(|_| "0.0.0.0:8988".to_string())
}
fn default_ws_addr() -> String {
    // P2: 从环境变量读取 WebSocket 地址
    std::env::var("WS_ADDR").unwrap_or_else(|_| "0.0.0.0:8089".to_string())
}
...
fn default_video_storage() -> String {
    // P2: 从环境变量读取视频存储路径
    std::env::var("VIDEO_STORAGE_PATH").unwrap_or_else(|_| "./data/videos".to_string())
}
...
fn default_legacy_host() -> String {
    // P2: 从环境变量读取遗留服务主机
    std::env::var("LEGACY_HOST").unwrap_or_else(|_| "127.0.0.1".to_string())
}
fn default_legacy_port() -> u16 {
    // P2: 从环境变量读取遗留服务端口
    std::env::var("LEGACY_PORT")
        .ok()
        .and_then(|s| s.parse().ok())
        .unwrap_or(9808)
}
    ...
    fn default() -> Self {
        Self {
            ...
            security: SecurityConfig {
                jwt_secret: default_jwt_secret(), // P2: 从环境变量读取
...
impl UnifiedConfig {
    ...
    fn validate(&self) -> AppResult<()> {
        ...
        if let Err(e) = self.architecture.validate() {
            return Err(AppError::validation(&format!(
                "Architecture config validation failed: {}",
                e
            )));
        }
    ...
    pub fn get_argon2_config(&self) -> Params {
        ...
        .unwrap_or_else(|e| {
            warn!("Argon2参数无效: {:?}, 使用安全的默认参数", e);
            // 安全的Argon2默认参数
            Params::new(65536, 3, 4, None).expect("即使使用默认值，Argon2参数也应该有效")
        })
    }
...
pub mod manager {
    ...
    pub fn get_config() -> UnifiedConfig {
        CONFIG
            .read()
            .expect("config should always be readable")
            .clone()
    }
