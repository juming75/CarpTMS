...
impl EnhancedPerformanceMonitor {
    ...
    pub fn record_system_metrics(&self, metrics: SystemMetrics) {
        ...
        // 存储历史数据
        if let Ok(mut h) = self.system_metrics_history.write() {
            h.push(metrics);
        }
    ...
    pub fn record_application_metrics(&self, metrics: ApplicationMetrics) {
        ...
        // 存储历史数据
        if let Ok(mut h) = self.app_metrics_history.write() {
            h.push(metrics);
        }
    ...
    pub fn record_database_metrics(&self, metrics: DatabaseMetrics) {
        ...
        // 存储历史数据
        if let Ok(mut h) = self.db_metrics_history.write() {
            h.push(metrics);
        }
    ...
    pub fn record_cache_metrics(&self, metrics: CacheMetrics) {
        ...
        // 存储历史数据
        if let Ok(mut h) = self.cache_metrics_history.write() {
            h.push(metrics);
        }
    ...
    pub fn record_business_metrics(&self, metrics: BusinessMetrics) {
        ...
        // 存储历史数据
        if let Ok(mut h) = self.business_metrics_history.write() {
            h.push(metrics);
        }
    ...
    pub fn get_system_metrics_history(&self, duration: Duration) -> Vec<SystemMetrics> {
        let cutoff_time =
            Utc::now() - chrono::Duration::from_std(duration).expect("duration out of range");
