...
pub struct ArenaAllocator {
    ...
    max_chunks: usize, // P1-4优化: 限制chunks数量防止内存泄漏
}
...
/// P1-4优化: Arena默认最大chunks数量
const DEFAULT_MAX_CHUNKS: usize = 10;

impl ArenaAllocator {
    /// 创建Arena分配器（默认最大chunks限制）
    pub fn new(chunk_size: usize) -> Self {
        Self::with_max_chunks(chunk_size, DEFAULT_MAX_CHUNKS)
    }

    /// P1-4优化: 创建Arena分配器，指定最大chunks数量
    pub fn with_max_chunks(chunk_size: usize, max_chunks: usize) -> Self {
        Self {
        ...
        if let Some(old_chunk) = current.replace(new_chunk) {
            if let Ok(mut chunks) = self.chunks.write() {
                if chunks.len() >= self.max_chunks {
                    ...
                    chunks.remove(0);
                    log::debug!(
                        "ArenaAllocator: 淘汰最旧的chunk，当前chunks: {}",
                        chunks.len()
                    );
                }
...
pub struct ArenaStats {
    ...
    pub max_chunks: usize, // P1-4: 最大允许chunks数
}
