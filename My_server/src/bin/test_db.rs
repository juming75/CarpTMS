...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\n测试查询车辆表...");
    let vehicles =
        sqlx::query("SELECT vehicle_id, vehicle_name, license_plate FROM vehicles LIMIT 5")
            .fetch_all(&pool)
            .await?;
    ...
    for vehicle in vehicles {
        println!(
            "{} | {} | {}",
            vehicle.get::<i32, &str>("vehicle_id"),
            vehicle.get::<String, &str>("vehicle_name"),
            vehicle.get::<String, &str>("license_plate")
        );
    }
    ...
    for column in user_columns {
        println!(
            "{} | {}",
            column.get::<String, &str>("column_name"),
            column.get::<String, &str>("data_type")
        );
    }
