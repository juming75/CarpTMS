...
use crate::domain::entities::statistics::{
    CustomStatistics, DeviceStatistics, StatisticsQuery, VehicleStatistics, WeighingStatistics,
};
...
impl StatisticsUseCases {
    ...
    ) -> Result<WeighingStatistics> {
        // 业务逻辑：设置默认时间范围为最近30天
        let end_time = query.end_time.unwrap_or(chrono::Utc::now());
        let start_time = query
            .start_time
            .unwrap_or(end_time - chrono::Duration::days(30));

        self.repository
            .get_weighing_statistics(start_time, end_time)
            .await
    }
    ...
    pub async fn get_custom_statistics(&self, query: StatisticsQuery) -> Result<CustomStatistics> {
        ...
        let start_time = query
            .start_time
            .unwrap_or(end_time - chrono::Duration::days(7));

        self.repository
            .get_custom_statistics(start_time, end_time)
            .await
    }
}
