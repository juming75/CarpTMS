...
mod tests {
    ...
    impl FinanceRepository for MockFinanceRepository {
        async fn get_finance_costs(
            &self,
            _query: FinanceCostQuery,
        ) -> Result<(Vec<FinanceCost>, i64), anyhow::Error> {
            Ok((vec![], 0))
        }

        async fn get_finance_cost(
            &self,
            _cost_id: i32,
        ) -> Result<Option<FinanceCost>, anyhow::Error> {
            Ok(None)
        ...
        async fn update_finance_cost(
            &self,
            _cost_id: i32,
            _cost: FinanceCostUpdate,
        ) -> Result<Option<FinanceCost>, anyhow::Error> {
            Ok(None)
        }

        async fn delete_finance_cost(&self, _cost_id: i32) -> Result<bool, anyhow::Error> {
            Ok(true)
        }

        async fn get_finance_invoices(
            &self,
            _query: FinanceInvoiceQuery,
        ) -> Result<(Vec<FinanceInvoice>, i64), anyhow::Error> {
            Ok((vec![], 0))
        }
        ...
        async fn update_finance_invoice(
            &self,
            _invoice_id: i32,
            _invoice: FinanceInvoiceUpdate,
        ) -> Result<Option<FinanceInvoice>, anyhow::Error> {
            Ok(None)
        }

        async fn delete_finance_invoice(&self, _invoice_id: i32) -> Result<bool, anyhow::Error> {
            Ok(true)
        }

        async fn get_finance_statistics(
            &self,
            _query: FinanceStatisticsQuery,
        ) -> Result<FinanceStatistics, anyhow::Error> {
    ...
    async fn test_create_finance_cost() {
        ...
        let cost = FinanceCostCreate {
            ...
            cost_date: NaiveDate::from_ymd_opt(2024, 1, 1)
                .unwrap()
                .and_hms_opt(0, 0, 0)
                .unwrap(),
        };
