...
use crate::domain::entities::statistic::{Statistic, StatisticCreateRequest, StatisticUpdateRequest, StatisticQuery, StatisticSummary, StatisticTrendPoint, StatisticNewParams};
...
impl StatisticUseCases {
    ...
    /// 批量创建统计数据
    pub async fn batch_create(&self, requests: &[StatisticCreateRequest]) -> AppResult<Vec<Statistic>> {
        let statistics: Vec<Statistic> = requests.iter().map(|request| {
            let params = StatisticNewParams {
...
mod tests {
    ...
    async fn test_create_statistic() -> Result<(), anyhow::Error> {
        ...
        let expected_statistic = Statistic::new(StatisticNewParams {
            stat_type: "weight".to_string(),
            value: 100.5,
            unit: "kg".to_string(),
            category: Some("truck".to_string()),
        ...
        mock_repo
            .expect_create()
            .returning(|stat| Ok(stat.clone()));
        
        let use_cases = StatisticUseCases::new(Arc::new(mock_repo));
        let result = use_cases.create(request).await;
        
        assert!(result.is_ok());
        let created_statistic = result.ok_or_else(|| AppError::resource_not_found("Failed to create statistic"))?;
