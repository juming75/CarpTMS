...
use crate::domain::entities::organization::{Organization, OrganizationCreateRequest, OrganizationUpdateRequest, OrganizationQuery};
...
impl OrganizationUseCases {
    ...
    /// 更新组织单位
    pub async fn update(&self, unit_id: &str, request: OrganizationUpdateRequest) -> AppResult<Organization> {
...
mod tests {
    ...
    async fn test_create_organization() -> Result<(), anyhow::Error> {
        ...
        let expected_organization = Organization::new(
            "org-001".to_string(),
        ...
        mock_repo
            .expect_create()
            .returning(|org| Ok(org.clone()));
        
        let use_cases = OrganizationUseCases::new(Arc::new(mock_repo));
        let result = use_cases.create(request).await;
        
        assert!(result.is_ok());
        let created_organization = result.ok_or_else(|| AppError::resource_not_found("Failed to create organization"))?;
