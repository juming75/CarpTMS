...
impl DeviceUseCases {
    ...
    pub async fn delete_device(&self, device_id: &str) -> Result<bool, anyhow::Error> {
        ...
        // 例如:检查该设备是否关联了车辆或称重数据,如果关联则不允许删除
        if device_id.is_empty() {
            return Err(anyhow::anyhow!("设备ID不能为空"));
        }
...
mod tests {
        ...
        async fn update_device(
            ...
            if let Some(existing_device) = self.devices.iter().find(|d| d.device_id == device_id) {
                ...
                let updated_device = Device {
                    ...
                    device_name: device
                        .device_name
                        .unwrap_or(existing_device.device_name.clone()),
                    device_type: device
                        .device_type
                        .unwrap_or(existing_device.device_type.clone()),
                    device_model: device
                        .device_model
                        .unwrap_or(existing_device.device_model.clone()),
                    manufacturer: device
                        .manufacturer
                        .unwrap_or(existing_device.manufacturer.clone()),
                    serial_number: device
                        .serial_number
                        .unwrap_or(existing_device.serial_number.clone()),
                    communication_type: device
                        .communication_type
                        .unwrap_or(existing_device.communication_type.clone()),
                    ...
                    install_address: device
                        .install_address
                        .or(existing_device.install_address.clone()),
                    install_technician: device
                        .install_technician
                        .or(existing_device.install_technician.clone()),
    ...
    async fn test_get_devices() {
        ...
        // 测试获取设备列表
        let (devices, total_count) = device_use_cases
            .get_devices(DeviceQuery::default())
            .await
            .unwrap();
    ...
    async fn test_create_device() {
        // 创建模拟仓库
        let mock_repo = Arc::new(MockDeviceRepository { devices: vec![] });
    ...
    async fn test_update_device() {
        ...
        let updated_device = device_use_cases
            .update_device("DEV001", device_update.clone())
            .await
            .unwrap();
        ...
        // 测试更新不存在的设备
        let updated_device = device_use_cases
            .update_device("DEV999", device_update.clone())
            .await
            .unwrap();
