...
use crate::domain::entities::openapi_platform::{OpenapiPlatform, OpenapiPlatformCreateRequest, OpenapiPlatformUpdateRequest, OpenapiPlatformQuery};
...
impl OpenapiPlatformUseCases {
    ...
    /// 创建 OpenAPI 平台
    pub async fn create(&self, request: OpenapiPlatformCreateRequest) -> AppResult<OpenapiPlatform> {
        // 验证请求
        request.validate()
    ...
    /// 获取所有 OpenAPI 平台
    pub async fn get_all(&self, query: &OpenapiPlatformQuery) -> AppResult<(Vec<OpenapiPlatform>, i64)> {
        self.repository.get_all(query).await
    }
    
    /// 更新 OpenAPI 平台
    pub async fn update(&self, id: i32, request: OpenapiPlatformUpdateRequest) -> AppResult<OpenapiPlatform> {
