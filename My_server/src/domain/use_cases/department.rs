...
pub trait DepartmentRepository: Send + Sync {
    ...
    async fn get_department(&self, department_id: i32)
...
impl DepartmentUseCases {
    ...
    pub async fn update_department(
        ...
        self.repository
            .update_department(department_id, department)
            .await
    }
    ...
    async fn execute(
        ...
        match command.command_type.as_str() {
            "get_departments" => {
                ...
                let (departments, total) = self.get_departments(query).await.map_err(|e| {
                    AppError::internal_error(&format!("Failed to get departments: {}", e), None)
                })?;
                let data = serde_json::json!({ "departments": departments, "total": total });
                Ok(CommandResult::success_with_data(vec![], data))
            }
            "get_department" => {
                let department_id = serde_json::from_value(command.data).map_err(|e| {
                    AppError::internal_error(&format!("Failed to parse department_id: {}", e), None)
                })?;
                let department = self.get_department(department_id).await.map_err(|e| {
                    AppError::internal_error(&format!("Failed to get department: {}", e), None)
                })?;
                let data = serde_json::json!({ "department": department });
                Ok(CommandResult::success_with_data(vec![], data))
            }
            "create_department" => {
                let department = serde_json::from_value(command.data).map_err(|e| {
                    AppError::internal_error(
                        &format!("Failed to parse department data: {}", e),
                        None,
                    )
                })?;
                let created_department = self.create_department(department).await.map_err(|e| {
                    AppError::internal_error(&format!("Failed to create department: {}", e), None)
                })?;
                ...
                let event = DomainEvent::new(
                    ...
                    "DepartmentCreated",
                    serde_json::to_value(created_department).map_err(|e| {
                        AppError::internal_error(
                            &format!("Failed to serialize department: {}", e),
                            None,
                        )
                    })?,
                    1,
            ...
            "update_department" => {
                let (department_id, department) =
                    serde_json::from_value(command.data).map_err(|e| {
                        AppError::internal_error(
                            &format!("Failed to parse update data: {}", e),
                            None,
                        )
                    })?;
                let updated_department = self
                    .update_department(department_id, department)
                    .await
                    .map_err(|e| {
                        AppError::internal_error(
                ...
                if let Some(department) = updated_department {
                    let event = DomainEvent::new(
                        ...
                        "DepartmentUpdated",
                        serde_json::to_value(department).map_err(|e| {
                            AppError::internal_error(
                                &format!("Failed to serialize department: {}", e),
                                None,
                            )
            ...
            "delete_department" => {
                ...
                let deleted = self.delete_department(department_id).await.map_err(|e| {
                    AppError::internal_error(&format!("Failed to delete department: {}", e), None)
                })?;
...
mod tests {
    ...
    impl DepartmentRepository for MockDepartmentRepository {
        ...
        async fn update_department(
            &self,
            _department_id: i32,
            _department: DepartmentUpdate,
        ) -> Result<Option<Department>, anyhow::Error> {
            Ok(None)
        }

        async fn delete_department(&self, _department_id: i32) -> Result<bool, anyhow::Error> {
            Ok(true)
        }

        async fn has_sub_departments(&self, _department_id: i32) -> Result<bool, anyhow::Error> {
            Ok(false)
        }
    }
