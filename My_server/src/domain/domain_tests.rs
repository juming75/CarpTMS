...
#[cfg(test)]
mod tests {
    use crate::domain::entities::vehicle::{Vehicle, VehicleCreate, VehicleUpdate, VehicleQuery};
    use crate::domain::entities::order::{Order, OrderCreate, OrderQuery};
    ...
    fn create_test_vehicle() -> Vehicle {
        Vehicle {
            ...
            status: 1,
            create_time: NaiveDateTime::parse_from_str("2024-01-01 00:00:00", "%Y-%m-%d %H:%M:%S").unwrap(),
            update_time: None,
            create_user_id: 1,
            update_user_id: None,
        }
    ...
    fn test_vehicle_create_entity() {
        let vehicle_create = VehicleCreate {
            ...
            register_date: NaiveDateTime::parse_from_str("2023-01-01 00:00:00", "%Y-%m-%d %H:%M:%S").unwrap(),
            inspection_date: NaiveDateTime::parse_from_str("2024-01-01 00:00:00", "%Y-%m-%d %H:%M:%S").unwrap(),
            insurance_date: NaiveDateTime::parse_from_str("2024-06-01 00:00:00", "%Y-%m-%d %H:%M:%S").unwrap(),
            seating_capacity: 3,
