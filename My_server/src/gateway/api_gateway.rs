    ...
    pub async fn start(&self) -> std::io::Result<()> {
        ...
        HttpServer::new(move || {
            ...
            App::new()
                ...
                .app_data(web::Data::new(server_state.calibration_service.clone()))
                .app_data(web::Data::new(server_state.weighing_data_service.clone()))
                ...
                .wrap(crate::app::build_cors(
                    &server_state.config.security.allowed_origins,
                ))
                .wrap(crate::middleware::request_header::default_header_middleware())
                .wrap(crate::middleware::metrics::MetricsMiddleware::new())
                .wrap(actix_web::middleware::Logger::default())  // Logger放在最后，避免干扰WebSocket
...
pub fn configure_api_gateway_routes(cfg: &mut web::ServiceConfig) {
    ...
    cfg.route(
        "/gateway/health",
        web::get().to(|| async { HttpResponse::Ok().body("API Gateway is healthy") }),
    );
}
