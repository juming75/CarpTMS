...
impl HandlerFactory {
    ...
    pub fn create_vehicle_handler(&self) -> Box<dyn RequestHandler> {
        ...
        match self.mode {
            ...
            ArchitectureMode::MicroDDD => {
                Box::new(MicroserviceVehicleHandler::new())
            }
        }
    }
    ...
    pub fn create_order_handler(&self) -> Box<dyn RequestHandler> {
        match self.mode {
            ArchitectureMode::MonolithDDD => {
                Box::new(MonolithOrderHandler::new(self.pool.clone()))
            }
            ArchitectureMode::MicroDDD => {
                Box::new(MicroserviceOrderHandler::new())
            }
        }
    }
}
    ...
    async fn handle_get(
        ...
        match parts.len() {
            2 => {
                ...
                let page_size = query.get("page_size").and_then(|v| v.as_i64()).unwrap_or(20) as i32;
