...
use crate::domain::entities::openapi_platform::{
    OpenapiPlatform, OpenapiPlatformQuery, OpenapiPlatformUpdateRequest,
};
...
impl OpenapiPlatformRepository for OpenapiPlatformRepositoryImpl {
    async fn create(&self, platform: &OpenapiPlatform) -> AppResult<OpenapiPlatform> {
        ...
        .await
        .map_err(|e| {
            AppError::db_error("Failed to create openapi platform", Some(&e.to_string()))
        })?;
    ...
    async fn get_by_id(&self, id: i32) -> AppResult<Option<OpenapiPlatform>> {
        ...
        .await
        .map_err(|e| {
            AppError::db_error("Failed to get openapi platform by id", Some(&e.to_string()))
        })?;
    ...
    async fn get_all(
        ...
        // 构建计数查询
        let count_query = format!("SELECT COUNT(*) FROM openapi_platforms {}", where_clause);
        ...
        let total: i64 = count_query_builder
            ...
            .await
            .map_err(|e| {
                AppError::db_error("Failed to count openapi platforms", Some(&e.to_string()))
            })?;
        ...
        let platforms = query_builder.fetch_all(&*self.db).await.map_err(|e| {
            AppError::db_error("Failed to get openapi platforms", Some(&e.to_string()))
        })?;
    ...
    async fn update(
        ...
        .await
        .map_err(|e| {
            AppError::db_error("Failed to update openapi platform", Some(&e.to_string()))
        })?;
    ...
    async fn update_status(&self, id: i32, status: &str) -> AppResult<OpenapiPlatform> {
        ...
        .map_err(|e| {
            AppError::db_error(
                "Failed to update openapi platform status",
                Some(&e.to_string()),
            )
        })?;
    ...
    async fn delete(&self, id: i32) -> AppResult<bool> {
        let result = sqlx::query("DELETE FROM openapi_platforms WHERE id = $1")
            .bind(id)
            .execute(&*self.db)
            .await
            .map_err(|e| {
                AppError::db_error("Failed to delete openapi platform", Some(&e.to_string()))
            })?;
