...
use crate::domain::entities::statistic::{
    Statistic, StatisticQuery, StatisticSummary, StatisticTrendPoint, StatisticUpdateRequest,
};
...
impl StatisticRepository for StatisticRepositoryImpl {
    ...
    async fn batch_create(&self, statistics: &[Statistic]) -> AppResult<Vec<Statistic>> {
        ...
        let result = query_builder.fetch_all(&*self.db).await.map_err(|e| {
            AppError::db_error("Failed to batch create statistics", Some(&e.to_string()))
        })?;
