...
use crate::domain::entities::organization::{Organization, OrganizationUpdateRequest, OrganizationQuery};
...
impl OrganizationRepository for OrganizationRepositoryImpl {
    ...
    async fn get_by_id(&self, unit_id: &str) -> AppResult<Option<Organization>> {
        ...
        .await
        .map_err(|e| AppError::db_error("Failed to get organization by id", Some(&e.to_string())))?;
    ...
    async fn get_by_ids(&self, unit_ids: &[String]) -> AppResult<Vec<Organization>> {
        ...
        let organizations = query_builder
            .fetch_all(&*self.db)
            .await
            .map_err(|e| AppError::db_error("Failed to get organizations by ids", Some(&e.to_string())))?;
    ...
    async fn get_all(&self, query: &OrganizationQuery) -> AppResult<(Vec<Organization>, i64)> {
        ...
        let total: i64 = count_query_builder
            .fetch_one(&*self.db)
            .await
            .map_err(|e| AppError::db_error("Failed to count organizations", Some(&e.to_string())))?;
        ...
        let organizations = query_builder
            .fetch_all(&*self.db)
            .await
            .map_err(|e| AppError::db_error("Failed to get organizations", Some(&e.to_string())))?;

        Ok((organizations, total))
    }

    async fn update(&self, _unit_id: &str, _organization: &OrganizationUpdateRequest) -> AppResult<Organization> {
        // 暂时返回错误，后续会实现完整的更新逻辑
    ...
    async fn update_status(&self, unit_id: &str, status: &str) -> AppResult<Organization> {
        ...
        .fetch_one(&*self.db)
        .await
        .map_err(|e| AppError::db_error("Failed to update organization status", Some(&e.to_string())))?;
    ...
    async fn delete(&self, unit_id: &str) -> AppResult<bool> {
        let result = sqlx::query(
            "DELETE FROM organizations WHERE unit_id = $1"
        )
        .bind(unit_id)
        .execute(&*self.db)
        .await
        .map_err(|e| AppError::db_error("Failed to delete organization", Some(&e.to_string())))?;
