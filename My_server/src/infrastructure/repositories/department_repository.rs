...
impl DepartmentRepository for DepartmentRepositoryImpl {
    ...
    async fn create_department(&self, department: DepartmentCreate) -> Result<Department, anyhow::Error> {
        ...
        // 获取父部门名称和经理名称
        let parent_name_query = r#"SELECT department_name FROM departments WHERE department_id = $1"#;
        let parent_department_name: Option<String> =
            if let Some(parent_id) = created_department.get::<Option<i32>, _>("parent_department_id") {
        ...
        let manager_name_query = r#"SELECT full_name FROM users WHERE user_id = $1"#;
