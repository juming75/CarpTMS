...
impl VehicleRepository for PgVehicleRepository {
    ...
    async fn get_vehicles(
        ...
        log::info!(
            "Getting vehicles with page={}, page_size={}, offset={}",
            page,
            page_size,
            offset
        );
        ...
        // 优化：使用明确的列名，避免 SELECT *
        let data_query = r#"
            SELECT 
                vehicle_id, vehicle_name, license_plate, vehicle_type, vehicle_color,
                vehicle_brand, vehicle_model, engine_no, frame_no, register_date,
                inspection_date, insurance_date, seating_capacity, load_capacity,
                vehicle_length, vehicle_width, vehicle_height, device_id, terminal_type,
                communication_type, sim_card_no, install_date, install_address,
                install_technician, own_no, own_name, own_phone, own_id_card,
                own_address, own_email, group_id, operation_status, operation_route,
                operation_area, operation_company, driver_name, driver_phone,
                driver_license_no, purchase_price, annual_fee, insurance_fee,
                remark, status, create_user_id, update_user_id, create_time, update_time
            FROM vehicles 
    ...
    async fn get_vehicles_batch(&self, vehicle_ids: &[i32]) -> Result<Vec<Vehicle>, anyhow::Error> {
        ...
        let vehicles = sqlx::query_as::<_, Vehicle>(
            ...
            SELECT 
                ...
                remark, status, create_user_id, update_user_id, create_time, update_time
            FROM vehicles 
            WHERE vehicle_id = ANY($1) AND status = 1"#,
        )
        .bind(vehicle_ids)
        .fetch_all(&*self.pool)
        .await?;
