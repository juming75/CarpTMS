...
use crate::domain::entities::statistics::{VehicleStatistics, DeviceStatistics, WeighingStatistics, CustomStatistics, StatisticsItem};
...
impl StatisticsRepository for StatisticsRepositoryImpl {
    ...
    async fn get_weighing_statistics(&self, start_time: DateTime<Utc>, end_time: DateTime<Utc>) -> Result<WeighingStatistics, anyhow::Error> {
        // 获取总称重数据
    ...
    async fn get_safety_ranking(&self) -> Result<Vec<serde_json::Value>, anyhow::Error> {
        ...
        if let Err(e) = sqlx::query(create_alerts_table)
            .execute(&*self.db)
            .await
        {
    ...
    async fn get_custom_statistics(&self, start_time: DateTime<Utc>, end_time: DateTime<Utc>) -> Result<CustomStatistics, anyhow::Error> {
        // 使用简单的固定查询,按天分组
