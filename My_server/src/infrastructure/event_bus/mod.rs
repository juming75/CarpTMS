            ...
            match self.receiver.recv().await {
                Ok(payload) => {
                    ...
                    let event: E = serde_json::from_slice(&payload).map_err(|e| {
                        crate::errors::AppError::internal_error(&format!("Failed to deserialize event: {}", e), None)
                    })?;
                    return Ok(event);
                }
                ...
                Err(tokio::sync::broadcast::error::RecvError::Closed) => {
                    return Err(crate::errors::AppError::internal_error("Event channel closed", None));
                }
            }
        }
    }
