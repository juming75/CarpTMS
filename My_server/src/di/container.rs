...
impl Container {
        ...
        F: Fn(&Container) -> Arc<T> + 'static + Send + Sync,
    {
        ...
        if let Ok(mut regs) = self.registrations.write() {
            ...
            info!("Registered singleton dependency for type: {:?}", std::any::type_name::<T>());
        } else {
        ...
        F: Fn(&Container) -> Arc<T> + 'static + Send + Sync,
    {
        ...
        if let Ok(mut regs) = self.registrations.write() {
            regs.insert(type_id, registration);
            info!("Registered transient dependency for type: {:?}", std::any::type_name::<T>());
        } else {
    ...
    pub fn resolve<T: Send + Sync + 'static>(&self) -> Option<Arc<T>> {
        ...
        warn!("Dependency not found for type: {:?}", std::any::type_name::<T>());
        None
    }

    /// 从注册信息中解析依赖
    fn resolve_registration<T: Send + Sync + 'static>(&self, registration: &DependencyRegistration) -> Option<Arc<T>> {
    ...
    /// 将 Any 类型转换为指定类型 T
    fn downcast_any_to<T: Send + Sync + 'static>(&self, any: Arc<dyn Any + Send + Sync>) -> Option<Arc<T>> {
        // 尝试直接 downcast
    ...
    pub fn unregister<T: Send + Sync + 'static>(&self) {
        ...
        if let Ok(mut regs) = self.registrations.write() {
            regs.remove(&type_id);
            info!("Unregistered dependency for type: {:?}", std::any::type_name::<T>());
        }
    }
