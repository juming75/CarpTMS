...
impl CacheConsistencyManager {
    ...
    pub fn get_invalidation_queue(&self) -> Vec<String> {
        self.invalidation_queue.read().ok().map(|q| q.clone()).unwrap_or_default()
    }
...
impl MemoryCacheLayer {
    ...
    fn evict_if_needed(&self) {
        ...
        let current_size = items.as_ref().map(|i| i.len()).unwrap_or(0);
        let threshold_size =
...
impl UnifiedCacheManager {
    ...
    pub fn get_stats(&self) -> CacheStats {
        self.cache_stats.read().ok().map(|s| s.clone()).unwrap_or_default()
    }
    ...
    pub fn get_cache_version(&self) -> String {
        self.cache_version.read().ok().map(|v| v.clone()).unwrap_or_default()
    }
