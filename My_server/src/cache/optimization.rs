    ...
    pub async fn get(&self, key: &str) -> AppResult<Option<Vec<u8>>> {
        ...
        if result.is_ok() {
            ...
            if let Some(record) = access_records.get_mut(key) {
                ...
                record.access_frequency = record.access_count as f64
                    / now.duration_since(record.last_access).as_secs_f64();
            } else {
        ...
        result.map_err(|e| crate::errors::AppError::internal_error(&e.to_string(), None))
    }
