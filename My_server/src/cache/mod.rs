...
pub use optimization::{CacheWarmer, EnhancedCacheManager, EnhancedCacheStats};
...
/// P1-5优化: 缓存版本号存储，用于一致性验证
static CACHE_VERSIONS: std::sync::LazyLock<
    std::sync::RwLock<std::collections::HashMap<String, u64>>,
> = std::sync::LazyLock::new(|| std::sync::RwLock::new(std::collections::HashMap::new()));

/// 获取缓存项版本号
pub fn get_cache_version(key: &str) -> Option<u64> {
    CACHE_VERSIONS.read().ok()?.get(key).copied()
}

/// 设置缓存项版本号
pub fn set_cache_version(key: &str, version: u64) {
    if let Ok(mut versions) = CACHE_VERSIONS.write() {
...
impl CacheManager {
    ...
    /// 获取缓存项
    /// P1-5优化: 首先从内存缓存中获取,若存在则验证版本一致性
    /// 版本不一致时从Redis获取最新数据并更新内存缓存
    pub async fn get(&self, key: &str) -> Result<Option<Vec<u8>>, anyhow::Error> {
        // P1-5优化: 首先从内存缓存中获取并验证版本
        if let Some(value) = self.memory_cache.get(key) {
            // 验证版本一致性
            if let Some(redis_cache) = self.redis_cache.lock().await.as_mut() {
                let redis_version = redis_cache.get_version(key).await.unwrap_or_else(|e| {
                    log::warn!("获取Redis缓存版本失败: {}", e);
                    0
                });
                let local_version = get_cache_version(key).unwrap_or(0);

                if redis_version > local_version {
                    // 版本过期，从Redis获取最新数据
                    log::debug!("Cache版本过期，刷新: {}", key);
                    if let Some(new_value) = redis_cache.get(key).await? {
                        self.memory_cache.set(
                            key,
                            new_value.clone(),
                            Some(Duration::from_secs(300)),
                        );
                        set_cache_version(key, redis_version);
        ...
        if let Some(redis_cache) = self.redis_cache.lock().await.as_mut() {
            if let Some(value) = redis_cache.get(key).await? {
                let version = redis_cache.get_version(key).await.unwrap_or_else(|e| {
                    log::warn!("获取Redis缓存版本失败: {}", e);
                    1
                });
                // P1-5优化: 同时更新内存缓存和版本号
