...
use crate::schemas::{
    LogisticsTrackCreate, LogisticsTrackCreateBatch, LogisticsTrackResponse, LogisticsTrackUpdate,
    OrderCreate as SchemaOrderCreate, OrderItemCreate as SchemaOrderItemCreate, OrderItemUpdate,
    OrderResponse, OrderUpdate as SchemaOrderUpdate, PagedResponse, VehicleTracksQuery,
};
...
async fn paged_result_to_response(
    ...
    for dto in result.items {
        ...
        responses.push(order_dto_to_response(
            dto,
            vehicle_name,
            license_plate,
            driver_name,
...
pub async fn create_order_item(
    ...
    let new_item: crate::application::dto::OrderItemDto =
        service.create_order_item(domain_item).await?;
...
pub async fn create_logistics_track(
    ...
    // 获取车辆信息用于响应
    let vehicle_result: Result<Option<(String, String)>, crate::errors::AppError> =
        service.get_vehicle_info(new_track.vehicle_id).await;
    let vehicle_name = vehicle_result
        .ok()
        .flatten()
        .map(|(name, _)| name)
        .unwrap_or_default();
...
pub async fn update_logistics_track(
    ...
    match result {
        Some(updated_track) => {
            // 获取车辆信息用于响应
            let vehicle_result: Result<Option<(String, String)>, crate::errors::AppError> =
                service.get_vehicle_info(updated_track.vehicle_id).await;
            let vehicle_name = vehicle_result
                .ok()
                .flatten()
                .map(|(name, _)| name)
            ...
            // 获取订单号用于响应
            let order: Option<crate::application::dto::OrderDto> =
                service.get_order(updated_track.order_id).await?;
            let order_no = order.map(|o| o.order_no).unwrap_or_default();
...
pub async fn create_logistics_tracks_batch(
    ...
    for track in results {
        let vehicle_result: Result<Option<(String, String)>, crate::errors::AppError> =
            service.get_vehicle_info(track.vehicle_id).await;
        let vehicle_name = vehicle_result
            .ok()
            .flatten()
            .map(|(name, _)| name)
...
pub async fn get_vehicle_tracks(
    ...
    for track in result.items {
        ...
        let order: Option<crate::application::dto::OrderDto> =
            service.get_order(track.order_id).await?;
        let order_no = order.map(|o| o.order_no).unwrap_or_default();
