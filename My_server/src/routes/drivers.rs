        ...
        Ok((drivers, total)) => {
            ...
            let driver_responses: Vec<DriverResponse> = drivers
                ...
                .map(|driver| DriverResponse {
                    ...
                    create_time: driver.create_time.to_string(),
                    update_time: driver.update_time.map(|t| t.to_string()),
                    license_no: driver.license_no,
                    license_type: driver.license_type,
                    license_expiry: driver.license_expiry.map(|d| d.to_string()),
                    id_card: driver.id_card,
                    address: driver.address,
                    emergency_contact: driver.emergency_contact,
                    emergency_phone: driver.emergency_phone,
                    hire_date: driver.hire_date.map(|d| d.to_string()),
                })
                .collect();
...
pub async fn create_driver(
    ...
    let create_request = DriverCreateRequest {
        ...
        status: driver.status,
        license_no: driver.license_no.clone(),
        license_type: driver.license_type.clone(),
        license_expiry: driver
            .license_expiry
            .clone()
            .and_then(|s| chrono::NaiveDate::parse_from_str(&s, "%Y-%m-%d").ok()),
        id_card: driver.id_card.clone(),
        address: driver.address.clone(),
        emergency_contact: driver.emergency_contact.clone(),
        emergency_phone: driver.emergency_phone.clone(),
        hire_date: driver
            .hire_date
            .clone()
            .and_then(|s| chrono::NaiveDate::parse_from_str(&s, "%Y-%m-%d").ok()),
    ...
    match driver_use_cases.create_driver(create_request).await {
        Ok(created_driver) => {
            ...
            let response = DriverResponse {
                ...
                license_number: created_driver.license_number,
                phone_number: created_driver.phone_number,
                email: created_driver.email,
                status: created_driver.status,
                create_time: created_driver.create_time.to_string(),
                update_time: created_driver.update_time.map(|t| t.to_string()),
                license_no: created_driver.license_no,
                license_type: created_driver.license_type,
                license_expiry: created_driver.license_expiry.map(|d| d.to_string()),
                id_card: created_driver.id_card,
                address: created_driver.address,
                emergency_contact: created_driver.emergency_contact,
                emergency_phone: created_driver.emergency_phone,
                hire_date: created_driver.hire_date.map(|d| d.to_string()),
            };
...
pub async fn update_driver(
    ...
    // 调用用例更新司机
    match driver_use_cases
        .update_driver(driver_id, update_request)
        .await
    {
        ...
        Err(e) => {
            ...
            if e.to_string().contains("司机有关联数据，无法删除") {
                Err(AppError::business_error(
                    "Cannot delete driver with associated data",
                    None,
                ))
            } else {
