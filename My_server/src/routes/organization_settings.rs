...
pub async fn get_organization_settings(
    ...
    info!(
        "Fetching organization settings for {}: {}",
        organization_id, setting_key
    );

    let value = service
        .get_organization_settings(&organization_id, &setting_key)
        .await?;
...
pub async fn update_organization_settings(
    ...
    let (organization_id, setting_key) = path.into_inner();
    info!(
        "Updating organization settings for {}: {}",
        organization_id, setting_key
    );

    let result = service
        .update_organization_settings(&organization_id, &setting_key, settings.into_inner())
        .await?;
    Ok(success_response_with_message(
