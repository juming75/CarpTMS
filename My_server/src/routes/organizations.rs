...
use crate::domain::entities::organization::{
    OrganizationCreateRequest, OrganizationQuery, OrganizationUpdateRequest,
};
...
fn to_org_response(
    org: crate::domain::entities::organization::Organization,
    ...
    OrganizationUnitResponse {
        ...
        status: org.status,
        create_time: Utc.from_utc_datetime(&org.create_time),
        update_time: org.update_time.map(|t| Utc.from_utc_datetime(&t)),
    }
}
...
pub async fn get_organizations(
    ...
    // 转换为响应格式
    let org_responses: Vec<OrganizationUnitResponse> =
        organizations.into_iter().map(to_org_response).collect();
...
pub async fn create_organization(
    ...
    // 调用用例创建组织单位
    let created_org = match organization_use_cases.create(create_request).await {
        Ok(org) => org,
        Err(e) => {
            error!("Failed to create organization, error: {}", e);
            return Err(e);
        }
    };

    // 转换为响应格式
    let response = to_org_response(created_org);
...
pub async fn get_organization(
    ...
    match org {
        Some(organization) => {
            let response = to_org_response(organization);
...
pub async fn update_organization(
    ...
    // 调用用例更新组织单位
    match organization_use_cases
        .update(&unit_id, update_request)
        .await
    {
        Ok(updated_org) => {
            let response = to_org_response(updated_org);
        ...
        Err(e) => {
            warn!(
                "Organization unit not found for update: {} - {:?}",
                unit_id_clone, e
            );
            Err(AppError::not_found_error(
                "Organization unit not found".to_string(),
...
pub async fn update_organization_status(
    ...
    // 调用用例更新组织单位状态
    match organization_use_cases
        .update_status(&unit_id, new_status)
        .await
    {
        Ok(updated_org) => {
            let response = to_org_response(updated_org);
