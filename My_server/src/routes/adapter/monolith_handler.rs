...
impl RouteAdapter for MonolithRouteHandler {
    ...
    async fn handle_get(
        ...
        if path.starts_with("/api/vehicles") {
            self.handle_vehicles(path, "GET", user_id, organization_id, Some(query), None)
                .await
        } else if path.starts_with("/api/orders") {
            self.handle_orders(path, "GET", user_id, organization_id, Some(query), None)
                .await
        } else {
    ...
    async fn handle_post(
        ...
        if path.starts_with("/api/vehicles") {
            self.handle_vehicles(path, "POST", user_id, organization_id, None, Some(body))
                .await
        } else if path.starts_with("/api/orders") {
            self.handle_orders(path, "POST", user_id, organization_id, None, Some(body))
                .await
        } else {
            let ctx = self.create_context(user_id, organization_id);
            Ok(HttpResponse::Created().json(serde_json::json!({
    ...
    async fn handle_put(
        ...
        if path.starts_with("/api/vehicles") {
            self.handle_vehicles(path, "PUT", user_id, organization_id, None, Some(body))
                .await
        } else if path.starts_with("/api/orders") {
            self.handle_orders(path, "PUT", user_id, organization_id, None, Some(body))
                .await
        } else {
            let ctx = self.create_context(user_id, organization_id);
            Ok(self.success_response(
    ...
    async fn handle_delete(
        ...
        if path.starts_with("/api/vehicles") {
            self.handle_vehicles(path, "DELETE", user_id, organization_id, None, None)
                .await
        } else if path.starts_with("/api/orders") {
            self.handle_orders(path, "DELETE", user_id, organization_id, None, None)
                .await
        } else {
            Ok(HttpResponse::NoContent().finish())
        }
    ...
    async fn handle_patch(
        ...
        if path.starts_with("/api/vehicles") {
            self.handle_vehicles(path, "PATCH", user_id, organization_id, None, Some(body))
                .await
        } else if path.starts_with("/api/orders") {
            self.handle_orders(path, "PATCH", user_id, organization_id, None, Some(body))
                .await
        } else {
            let ctx = self.create_context(user_id, organization_id);
            Ok(self.success_response(
