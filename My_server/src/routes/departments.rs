...
use crate::domain::entities::department::{
    DepartmentCreateRequest, DepartmentQuery, DepartmentUpdateRequest,
};
...
pub async fn get_departments(
    ...
    match department_use_cases.get_departments(domain_query).await {
        ...
        Err(e) => Err(AppError::internal_error(
            &format!("Failed to fetch departments: {:?}", e),
            None,
        )),
    }
}
...
pub async fn get_department(
    ...
    match department_use_cases.get_department(department_id).await {
        ...
        Err(e) => Err(AppError::internal_error(
            &format!("Failed to fetch department: {:?}", e),
            None,
        )),
    }
}
...
pub async fn update_department(
    ...
    // 调用用例更新部门
    match department_use_cases
        .update_department(department_id, update_request)
        .await
    {
        ...
        Err(e) => {
            if e.to_string().contains("Department not found") {
                ...
                .to_string()
                .contains("Department cannot be its own parent")
            {
                Err(AppError::business_error(
...
pub async fn delete_department(
    ...
    match department_use_cases.delete_department(department_id).await {
        ...
        Err(e) => {
            if e.to_string()
                .contains("Cannot delete department with sub-departments")
            {
                Err(AppError::business_error(
