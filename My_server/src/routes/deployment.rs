...
pub async fn get_deployment_status() -> AppResult<HttpResponse> {
    ...
    let current_env =
        std::env::var("DEPLOYMENT_CURRENT_ENVIRONMENT").unwrap_or_else(|_| "blue".to_string());
...
pub async fn health_check(path: web::Path<String>) -> AppResult<HttpResponse> {
...
pub async fn switch_traffic(req: web::Json<SwitchTrafficRequest>) -> AppResult<HttpResponse> {
...
pub async fn rollback(req: web::Json<RollbackRequest>) -> AppResult<HttpResponse> {
    let current =
        std::env::var("DEPLOYMENT_CURRENT_ENVIRONMENT").unwrap_or_else(|_| "blue".to_string());
