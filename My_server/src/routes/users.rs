...
use crate::domain::entities::user::{UserCreate as DomainUserCreate, UserQuery as DomainUserQuery, UserUpdate as DomainUserUpdate};
use crate::errors::{
...
pub async fn create_user(
    ...
    let response = UserResponse {
        ...
        last_login_time: created_user.last_login_time.map(|t| Utc.from_utc_datetime(&t)),
...
pub async fn get_user(user_service: web::Data<Arc<UserServiceImpl>>, user_id: web::Path<i32>) -> AppResult<HttpResponse> {
...
pub async fn update_user(
    ...
    let domain_user_update = DomainUserUpdate {
        ...
        user_group_id: user.user_group_id,
        status: user.status, // 转换为i16
        last_login_time: None, // 暂时不更新最后登录时间
    };

    // 调用服务更新用户
    let updated_user = user_service.update_user(user_id, domain_user_update).await?;
...
pub fn configure_users_routes(cfg: &mut web::ServiceConfig) {
    ...
    cfg.route("/users", web::get().to::<_, (web::Data<Arc<UserServiceImpl>>, web::Query<UserQuery>)>(get_users))
        .route("/users", web::post().to::<_, (web::Data<Arc<UserServiceImpl>>, web::Json<UserCreate>)>(create_user))
        .route("/users/{user_id}", web::get().to::<_, (web::Data<Arc<UserServiceImpl>>, web::Path<i32>)>(get_user))
        .route("/users/{user_id}", web::put().to::<_, (web::Data<Arc<UserServiceImpl>>, web::Path<i32>, web::Json<UserUpdate>)>(update_user))
        .route("/users/{user_id}", web::delete().to::<_, (web::Data<Arc<UserServiceImpl>>, web::Path<i32>)>(delete_user))
        .route(
            "/users/{user_id}/permissions",
            web::post().to::<_, (web::Data<Arc<UserServiceImpl>>, web::Path<i32>, web::Json<Vec<(String, String)>>)>(assign_user_permissions),
        );
}
