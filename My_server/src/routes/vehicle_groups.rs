...
use crate::domain::entities::vehicle_group::{VehicleGroupCreateRequest, VehicleGroupUpdateRequest, VehicleGroupQuery};
use crate::errors::{created_response_with_message, success_response_with_message, AppError, AppResult};
...
fn domain_to_response(group: crate::domain::entities::vehicle_group::VehicleGroup) -> VehicleGroupResponse {
...
pub async fn get_vehicle_groups(
    ...
    // 4. 转换为 HTTP 响应
    let group_responses: Vec<VehicleGroupResponse> = groups
        .into_iter()
        .map(domain_to_response)
        .collect();
...
pub async fn create_vehicle_group(
    ...
    info!("Vehicle group created successfully: id={}, name={}", response.group_id, response.group_name);
    Ok(created_response_with_message("Vehicle group created successfully", response))
}
...
pub async fn get_vehicle_group(
    ...
    match group {
        Some(g) => {
            ...
            Ok(success_response_with_message("Vehicle group fetched successfully", response))
        }
        None => Err(AppError::not_found_error("Vehicle group not found".to_string())),
    }
}
...
pub async fn update_vehicle_group(
    ...
    // 2. 调用 use_cases 执行业务逻辑
    let updated_group = use_cases.update_vehicle_group(*group_id, update_data).await?;
    ...
    match updated_group {
        Some(g) => {
            // 4. 转换为 HTTP 响应
            let response = domain_to_response(g);
            Ok(success_response_with_message("Vehicle group updated successfully", response))
        }
        None => Err(AppError::not_found_error("Vehicle group not found".to_string())),
    }
}
...
pub async fn delete_vehicle_group(
    ...
    if result {
        ...
        Ok(success_response_with_message("Vehicle group deleted successfully", ()))
    } else {
        Err(AppError::not_found_error("Vehicle group not found".to_string()))
    }
}
...
pub async fn get_vehicle_group_tree(use_cases: web::Data<Arc<VehicleGroupUseCases>>) -> AppResult<HttpResponse> {
