...
use crate::domain::entities::openapi_platform::{OpenapiPlatformCreateRequest, OpenapiPlatformUpdateRequest, OpenapiPlatformQuery};
...
pub async fn update_openapi_platform_status(
    ...
    // 调用用例更新 OpenAPI 平台状态
    match openapi_platform_use_cases.update_status(id, new_status).await {
        Ok(updated_platform) => {
            // 转换为响应格式
            let response = OpenapiPlatformResponse {
                id: updated_platform.id,
...
pub async fn delete_openapi_platform(
    ...
    match openapi_platform_use_cases.delete(id).await {
        ...
        Ok(false) | Err(_) => {
            warn!(
                "OpenAPI platform not found for deletion: {}",
                id
            );
            Err(AppError::not_found_error(
                "OpenAPI platform not found".to_string(),
...
pub fn configure_openapi_platforms_routes(cfg: &mut web::ServiceConfig) {
    cfg.route("/openapi/platforms", web::get().to(get_openapi_platforms))
        .route("/openapi/platforms", web::post().to(create_openapi_platform))
        .route("/openapi/platforms/{id}", web::get().to(get_openapi_platform))
        .route(
