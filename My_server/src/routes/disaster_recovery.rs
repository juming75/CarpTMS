...
pub async fn list_backups() -> AppResult<HttpResponse> {
    ...
    let backup_ids = storage.list_backups().await
...
pub async fn create_backup(
    ...
    let output = tokio::process::Command::new("pg_dump")
        .args([
            "--dbname", &db_url,
            "--format", "c",
            "--file", &format!("{}/{}.backup", backup_path, backup_id),
            "--verbose",
        ])
        .output()
        .await
    ...
    if !output.status.success() {
        ...
        return Err(AppError::internal_error(&format!("备份失败: {}", stderr), None));
    }

    Ok(success_response_with_message("备份创建成功", serde_json::json!({
        "backup_id": backup_id,
...
pub async fn restore_backup(
    ...
    if !std::path::Path::new(&backup_file).exists() {
        return Err(AppError::not_found_error(format!("备份文件不存在: {}", backup_id)));
    }
    ...
    Ok(success_response_with_message("恢复完成", serde_json::json!({
        "backup_id": backup_id,
...
pub async fn delete_backup(
    ...
    Ok(success_response_with_message("备份删除成功", serde_json::json!({
        "backup_id": backup_id,
    })))
}
