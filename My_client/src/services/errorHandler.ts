...
if (typeof window !== 'undefined') {
  ...
  window.addEventListener('error', (event) => {
    // 完全忽略 ResizeObserver 无害错误，不打印任何日志
    if (isResizeObserverLoopError(event.error || new Error(event.message))) {
      event.preventDefault();
      return;
    }
    const error = ErrorHandler.handle(event.error || new Error(event.message));
    console.warn('[ErrorHandler] 未捕获的错误:', error);
  });
}
