...
import { throttle, MemoryMonitor } from '@/utils/performance';
...
const cleanupMap = () => {
  ...
  if (memoryMonitor) {
    ...
    console.log('[MapMonitor] 内存报告:', memoryMonitor.getReport());
    memoryMonitor = null;
  }
...
const initMap = () => {
  ...
  try {
    // 使用统一的地图配置获取天地图API Key
    const tiandituKey = getTiandituKey();
    if (!tiandituKey) {
      console.error('天地图 API 密钥未配置，请在 .env.local 中设置 VITE_TIANDITU_KEY');
      return;
    }
