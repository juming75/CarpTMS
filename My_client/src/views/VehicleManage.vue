<template>
  <div class="vehicle-manage">
    <el-card>
      ...
      <!-- 车辆列表 -->
      <el-table :data="vehicleList" v-loading="loading" @selection-change="handleSelectionChange" stripe>
        <el-table-column type="selection" width="55" />
        <el-table-column prop="vehicle_id" label="ID" width="80" />
        ...
        <el-table-column prop="own_name" label="车主" width="120" />
        <el-table-column prop="own_phone" label="联系电话" width="130" />
...
const loadVehicles = async () => {
  ...
  try {
    ...
    const response = (await vehicleApi.getAll()) as unknown as VehicleApiResponse;
    // 处理API响应
    if (response) {
      ...
      if ('success' in response && 'data' in response) {
        ...
        if (apiResponse.success && apiResponse.data) {
          // 检查data是否是分页响应格式
          const data = apiResponse.data as { items?: Vehicle[]; total?: number; length?: number };
          if ('items' in data) {
            // 分页响应格式
            vehicleList.value = data.items || [];
        ...
        // 直接返回数据数组
        const directResponse = response as unknown as Vehicle[];
