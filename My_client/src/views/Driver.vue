...
const fetchDrivers = async () => {
  ...
  try {
    ...
    const params = {
      ...
      driver_name: driverQueryForm.value.name,
      license_number: '',
      status: driverQueryForm.value.status ? statusMap[driverQueryForm.value.status] : undefined,
    };
    console.log('请求司机列表参数:', params);
    // SECURITY
    const response = await api.get('/api/drivers', { params });
    console.log('获取司机列表响应:', response);
    if (response && response.data && response.data.list) {
      driverList.value = response.data.list.map((item) => ({
        id: item.driver_id,
        ...
        joinDate: item.create_time,
        lastLogin: item.update_time,
      }));
      driverTotal.value = response.data.total || 0;
    } else {
      driverList.value = [];
      driverTotal.value = 0;
    }
    ...
    console.error('错误详情:', (error as { response?: { data: unknown }; message?: string }).response?.data || (error as { message?: string }).message || error);
...
const handleDriverView = async (row: DriverItem) => {
  try {
    const response = await api.get(`/api/drivers/${row.id}`);
    console.log('获取司机详情响应:', response);
    if (response && response.data) {
      selectedDriver.value = {
        id: response.data.driver_id,
        name: response.data.driver_name,
        phone: response.data.phone_number,
        license: response.data.license_number,
        licenseType: '',
        licenseExpiry: '',
        licenseIssue: '',
        status: response.data.status === 1 ? 'active' : 'inactive',
        joinDate: response.data.create_time,
        lastLogin: response.data.update_time,
        remark: '',
        gender: 'male',
        age: ''
      };
