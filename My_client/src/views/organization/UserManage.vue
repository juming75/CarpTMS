...
const handleSave = async () => {
  try {
    // 验证必填字段
    if (!editingUser.value.username || editingUser.value.username.length < 3) {
      ElMessage.warning('用户名必须至少3个字符');
      return;
    }
    
    if (!editingUser.value.full_name || editingUser.value.full_name.length < 2) {
      ElMessage.warning('真实姓名必须至少2个字符');
      return;
    }
    
    if (!editingUser.value.user_group_id) {
      ElMessage.warning('请选择用户角色');
      return;
    }

    const userData: Record<string, unknown> = {};
    if (editingUser.value.username) userData.username = editingUser.value.username;
    if (editingUser.value.full_name) userData.full_name = editingUser.value.full_name;
    if (editingUser.value.phone_number) userData.phone_number = editingUser.value.phone_number;
    if (editingUser.value.email) userData.email = editingUser.value.email;
    if (editingUser.value.user_group_id) userData.user_group_id = editingUser.value.user_group_id;
    if (editingUser.value.department_id) userData.department_id = editingUser.value.department_id;
    userData.status = editingUser.value.status ?? 1;

    if (!editingUser.value.id) {
      if (!editingUser.value.password || editingUser.value.password.length < 8) {
        ElMessage.warning('新用户必须设置密码（至少8个字符）');
        return;
      }
      userData.password = editingUser.value.password;
    } else if (editingUser.value.password && editingUser.value.password.length >= 8) {
      userData.password = editingUser.value.password;
    }
    ...
    ElMessage.error('保存用户失败: ' + (error instanceof Error ? error.message : String(error)));
  }
};
