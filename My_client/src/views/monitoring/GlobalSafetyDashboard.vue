...
import api from '@/api';
import { throttle, BatchUpdater, MemoryMonitor } from '@/utils/performance';
...
let alertsBatchUpdater: BatchUpdater<Alert> | null = null;
...
onMounted(async () => {
  ...
  // 初始化批量更新器 - 减少告警列表重渲染
  alertsBatchUpdater = new BatchUpdater<Alert>((newAlerts) => {
    // 批量更新告警列表
    displayedAlerts.value = [...alerts.value];
  }, 16); // 约16ms一帧
  ...
  console.log('[GlobalSafetyDashboard] 性能监控已启用');
});
...
onUnmounted(() => {
  ...
  if (memoryMonitor) {
    ...
    console.log('[GlobalSafetyDashboard] 内存报告:', memoryMonitor.getReport());
    memoryMonitor = null;
  }
  
  // 清理批量更新器
  if (alertsBatchUpdater) {
    alertsBatchUpdater.clear();
    alertsBatchUpdater = null;
  }

  // 移除事件监听
  // 注意：handleResizeThrottled 在闭包中，需要移除原监听器
  window.removeEventListener('resize', handleResize);

  // 清理 ECharts 图表资源 - 防止内存泄漏
  if (alertTrendChart) {
  ...
  // 清理 OpenLayers 地图资源 - 防止内存泄漏
  if (mapInstance.value) {
    // 移除所有图层
    const layers = mapInstance.value.getLayers();
    if (layers && typeof layers.getArray === 'function') {
  ...
  console.log('GlobalSafetyDashboard 组件卸载完成，内存已清理');
});
</script>
